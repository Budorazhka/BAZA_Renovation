...
async function main() {
  ...
  for (let i = 0; i < protectedStrings.length; i += 50) {
    ...
    try {
      const res = await translate(chunk, { from: 'ru', to: 'es' });
      ...
      for (const str of chunk) {
        try {
          await delay(200);
          const res = await translate(str, { from: 'ru', to: 'es' });
  ...
  fs.writeFileSync('es_dict.json', JSON.stringify(outData, null, 2), 'utf8');
  console.log('Finished es_dict.json');
}
