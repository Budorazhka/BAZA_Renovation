...
function TrendBadge({ trendPercent, size = 'sm' }: { trendPercent: number; size?: 'sm' | 'lg' }) {
  ...
  return (
    <span
      className={cn(
        size === 'lg'
          ? 'inline-flex items-center gap-1.5 whitespace-nowrap text-[20px] font-normal leading-none'
          : 'inline-flex items-center gap-1 whitespace-nowrap text-[10px] font-normal leading-none',
        trendTextColor
      )}
    >
  ...
  const cardContent = (
    <div className={cn('flex h-full min-h-[62px] flex-col items-center justify-center gap-1 text-center', className)}>
      <div className="flex min-w-0 items-center justify-center gap-1.5 text-[11px] font-medium text-slate-500">
        ...
        <span className="truncate">{t('overview.referralsStatCard.рефералы_l1_l2')}</span>
      </div>

      <div className="flex min-w-0 items-end justify-center gap-3">
        <div className="flex min-w-0 flex-col items-center gap-0.5">
          <span className="text-[10px] font-normal uppercase leading-none tracking-wide text-slate-500">L1</span>
          <span className="truncate text-[26px] font-normal leading-none tracking-tight text-slate-900">
        ...
        <div className="flex min-w-0 flex-col items-center gap-0.5">
          <span className="text-[10px] font-normal uppercase leading-none tracking-wide text-slate-500">L2</span>
          <span className="truncate text-[26px] font-normal leading-none tracking-tight text-slate-900">
      ...
      <p className="truncate text-[10px] font-medium leading-none text-slate-400">
        {t('overview.referralsStatCard.добавлено_рефералов')}</p>
    </div>
  )
        ...
        >
          <div className="grid grid-cols-2 gap-3 border-b border-slate-200 pb-1.5">
            <div className="flex flex-col items-center gap-1">
              <span className="text-[10px] font-normal uppercase leading-none text-slate-500">L1</span>
              {typeof trendPercentL1 === 'number' ? (
                <TrendBadge trendPercent={trendPercentL1} size="lg" />
              ) : (
                <span className="text-[20px] font-normal leading-none text-slate-400">—</span>
              )}
            </div>
            <div className="flex flex-col items-center gap-1">
              <span className="text-[10px] font-normal uppercase leading-none text-slate-500">L2</span>
