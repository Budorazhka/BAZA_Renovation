    ...
    <section className="overflow-hidden rounded-3xl border border-slate-300 bg-white shadow-sm">
      <header className="border-b border-slate-800 bg-slate-900 px-5 py-4 text-white">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <h2 className="text-3xl font-normal leading-tight">{t('overview.globeMap.карта_сети_mlm')}</h2>
          </div>

          <div className="grid grid-cols-3 gap-2">
            <div className="rounded-xl border border-slate-700 bg-slate-800/80 px-3 py-2">
              <p className="text-xs text-slate-300">{t('overview.globeMap.городов')}</p>
              <p className="text-xl font-normal">{networkStats.cityCount}</p>
            </div>
            <div className="rounded-xl border border-slate-700 bg-slate-800/80 px-3 py-2">
              <p className="text-xs text-slate-300">{t('overview.globeMap.рефералов')}</p>
              <p className="text-xl font-normal">{networkStats.referralsCount}</p>
            </div>
            <div className="rounded-xl border border-slate-700 bg-slate-800/80 px-3 py-2">
              <p className="text-xs text-slate-300">{t('overview.globeMap.выручка')}</p>
              <p className="text-xl font-normal">{formatMoney(networkStats.revenue)}</p>
            </div>
          </div>
        </div>
      </header>
        ...
        <div className="p-4">
          <div className="mb-3 flex flex-wrap items-center gap-2">
            {countryOptions.map((country) => {
              ...
              return (
                <button
                  ...
                  className={[
                    'inline-flex items-center gap-2 rounded-full border px-3 py-1.5 text-sm font-normal transition',
                    isActive
            ...
            <button
              ...
              className="rounded-full border border-slate-300 bg-white px-4 py-1.5 text-sm font-normal text-slate-700 transition hover:bg-slate-100"
            >
              {t('overview.globeMap.сбросить_фокус')}</button>
          </div>
          ...
          >
                      ...
                      >
                        ...
                        {showLabel && (
                          ...
                          >
                            ...
                            >
                              {city.partnersCount} {t('overview.globeMap.рефералов')}</text>
                          </g>
                        )}
                      </g>
                    </Marker>
            ...
            {hoveredCountryStats && hoveredCountryName && (
              <div className="pointer-events-none absolute right-3 top-3 rounded-lg border border-slate-500/50 bg-slate-900/90 px-3 py-2 text-xs text-slate-100 shadow-xl backdrop-blur-sm">
                <p className="text-[11px] font-normal uppercase tracking-wide text-slate-300">{hoveredCountryName}</p>
                <p className="mt-1 text-sm font-normal">{formatMoney(hoveredCountryStats.revenue)}</p>
                <p className="text-[11px] text-slate-300">{hoveredCountryStats.referralsCount} {t('overview.globeMap.рефералов')}</p>
              </div>
            )}
            ...
            <div className="pointer-events-none absolute bottom-3 left-3 rounded-xl border border-slate-500/50 bg-slate-900/80 px-3 py-2 text-xs text-slate-200 shadow-lg">
              {t('overview.globeMap.наведите_на_город_дл')}</div>
          </div>
        </div>

        <aside className="border-t border-slate-200 bg-slate-50 p-4 xl:border-l xl:border-t-0">
          <div className="rounded-2xl border border-slate-200 bg-white p-4">
            <p className="text-xs uppercase tracking-wide text-slate-500">{t('overview.globeMap.текущий_фокус')}</p>
            <h3 className="mt-1 text-2xl font-normal text-slate-900">{focusedCountry.name}</h3>

            <div className="mt-4 grid grid-cols-2 gap-2">
              <div className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2">
                <div className="flex items-center gap-1 text-slate-600">
                  <Building2 className="h-4 w-4" />
                  <span className="text-xs">{t('overview.globeMap.городов')}</span>
                </div>
                <p className="mt-1 text-2xl font-normal text-slate-900">
              ...
              <div className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2">
                <div className="flex items-center gap-1 text-slate-600">
                  ...
                  <span className="text-xs">{t('overview.globeMap.рефералов')}</span>
                </div>
                <p className="mt-1 text-2xl font-normal text-slate-900">
            ...
            <div className="mt-2 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2">
              <div className="flex items-center gap-1 text-slate-600">
                ...
                <span className="text-xs">{t('overview.globeMap.выручка_страны')}</span>
              </div>
              <p className="mt-1 text-2xl font-normal text-slate-900">
            ...
            {focusedCountryCities.map((city) => {
              ...
              return (
                <button
                  ...
                  title={t('overview.globeMap.клик_выбрать_город_п')}
                  className={[
                ...
                >
                  <div className="flex items-center justify-between gap-2">
                    <div className="min-w-0">
                      <p className="truncate text-lg font-normal">{city.name}</p>
                      <p
                        className={[
                          'text-sm',
                      ...
                      >
                        {city.partnersCount} {t('overview.globeMap.рефералов')}</p>
                      <p
                        className={[
                      ...
                      >
                        {t('overview.globeMap.клик_выбрать_еще_кли')}</p>
                    </div>
                    <div
                      className={[
                        'shrink-0 rounded-full border px-2 py-0.5 text-xs font-normal transition',
                        isActive
          ...
          <div className="mt-3 rounded-2xl border border-slate-200 bg-white p-4">
            ...
            >
              {t('overview.globeMap.открыть_аналитику_го')}</Button>
          </div>
        </aside>
      </div>
    </section>
