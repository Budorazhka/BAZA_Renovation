      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            {t('management.addPartnerDialog.добавить_партнера')}</DialogTitle>
          <DialogDescription>
            {t('management.addPartnerDialog.логин_партнера_обяза')}</DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
          <div className="space-y-1.5">
            <Label htmlFor="partner-login" className="text-sm font-medium">
              {t('management.addPartnerDialog.логин_партнера')}</Label>
            <Input
              id="partner-login"
              value={login}
              onChange={(event) => setLogin(event.target.value)}
              placeholder={t('management.addPartnerDialog.например_partner01_t')}
            />
          </div>

          <div className="space-y-1.5">
            <Label htmlFor="partner-name" className="text-sm font-medium">
              {t('management.addPartnerDialog.имя_партнера')}</Label>
            <Input
              id="partner-name"
              value={name}
              onChange={(event) => setName(event.target.value)}
              placeholder={t('management.addPartnerDialog.опционально')}
            />
          </div>

          <div className="space-y-1.5">
            <Label className="text-sm font-medium">{t('management.addPartnerDialog.тип')}</Label>
            <div className="flex gap-2">
              ...
              >
                {t('management.addPartnerDialog.мастер_партнер')}</Button>
              <Button
                variant={type === 'sub' ? 'default' : 'outline'}
                size="sm"
                onClick={() => setType('sub')}
              >
                {t('management.addPartnerDialog.суб_партнер')}</Button>
            </div>
          </div>

          {type === 'sub' && existingMasters.length > 0 && (
            <div className="space-y-1.5">
              <Label className="text-sm font-medium">{t('management.addPartnerDialog.привязать_к_мастеру')}</Label>
          ...
          {type === 'sub' && existingMasters.length === 0 && (
            <p className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-800">
              {t('management.addPartnerDialog.сначала_добавьте_хот')}</p>
          )}

          <div className="space-y-1.5">
            <Label className="text-sm font-medium">{t('management.addPartnerDialog.роли')}</Label>
        ...
        <DialogFooter className="gap-2 sm:gap-0">
          <Button variant="outline" onClick={() => handleOpenChange(false)}>
            {t('management.addPartnerDialog.отмена')}</Button>
          <Button onClick={handleSubmit} disabled={!isValid}>
            {t('management.addPartnerDialog.добавить')}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  )
