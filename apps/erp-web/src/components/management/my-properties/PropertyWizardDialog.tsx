  ...
  return (
    <label className="space-y-1">
      <div className="flex items-center justify-between gap-3">
        <span className="text-[12px] font-normal text-[color:var(--workspace-text)]">
          {label}
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div>
          <div className={cn(compact ? 'text-[13px]' : 'text-[14px]', 'font-normal leading-tight text-[color:var(--workspace-text)]')}>{title}</div>
    ...
    <label className="block rounded-[20px] border border-dashed border-[var(--hub-card-border)] bg-[color-mix(in_srgb,var(--rail-bg)_80%,transparent)] p-4 transition-colors hover:border-[rgba(52,211,153,0.45)]">
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0">
          <div className="text-[13px] font-normal text-[color:var(--workspace-text)]">{title}</div>
...
export function PropertyWizardDialog({
  ...
  const { t } = useI18n()
  const resolvedDefaults = useMemo((): Partial<PropertyWizardValues> | undefined => {
    if (mode !== 'create' || !defaults) return defaults
    if (defaults.category === 'primary') {
      return { ...defaults, category: 'secondary' }
    }
    return defaults
  }, [mode, defaults])
  ...
  const categoryOptions = useMemo(
    () =>
      mode === 'create'
        ? PROPERTY_CATEGORY_OPTIONS.filter((option) => option.value !== 'primary')
        : PROPERTY_CATEGORY_OPTIONS,
    [mode],
  )

  const [values, setValues] = useState<PropertyWizardValues>(() =>
    createPropertyWizardValues(property ?? undefined, resolvedDefaults),
  )
  ...
  useEffect(() => {
    if (!open) return
    const initialValues = createPropertyWizardValues(property ?? undefined, resolvedDefaults)
    setValues(initialValues)
    setActiveStepIndex(mode === 'edit' ? 1 : 0)
    setIsMapModalOpen(false)
  }, [open, property, resolvedDefaults, mode])
    ...
    return (
      <div className="space-y-4">
        <section className="space-y-2.5">
          <div>
            <h3 className="text-[15px] font-normal text-[color:var(--workspace-text)]">{t('management.my-properties.propertyWizardDialog.1_сегмент_объекта')}</h3>
          </div>
        ...
        <section className="space-y-2.5">
          <div>
            <h3 className="text-[15px] font-normal text-[color:var(--workspace-text)]">{t('management.my-properties.propertyWizardDialog.2_тип_карточки')}</h3>
          </div>
          <div className="grid gap-3 md:grid-cols-2">
        ...
        <section className="grid gap-3 md:grid-cols-3">
          <FieldShell label={t('management.my-properties.propertyWizardDialog.страна')} required>
            <Select value={values.country} onValueChange={handleCountryChange}>
              <SelectTrigger className={SELECT_CLASS}>
                <SelectValue placeholder={t('management.my-properties.propertyWizardDialog.выберите_страну')} />
              </SelectTrigger>
              <SelectContent>
          ...
          <FieldShell label={t('management.my-properties.propertyWizardDialog.город')} required>
            <Select value={values.city} onValueChange={handleCityChange}>
              <SelectTrigger className={SELECT_CLASS}>
                <SelectValue placeholder={t('management.my-properties.propertyWizardDialog.выберите_город')} />
              </SelectTrigger>
              <SelectContent>
          ...
          <FieldShell label={t('management.my-properties.propertyWizardDialog.статус')} hint="Можно скорректировать позже">
            <Select value={values.status} onValueChange={(status) => updateValue('status', status as SaleStatus)}>
              <SelectTrigger className={SELECT_CLASS}>
                <SelectValue placeholder={t('management.my-properties.propertyWizardDialog.выберите_статус')} />
              </SelectTrigger>
              <SelectContent>
    ...
    return (
      <div className="space-y-4">
        <section className="grid gap-3 md:grid-cols-2">
          <FieldShell label={t('management.my-properties.propertyWizardDialog.жк_комплекс')} required>
            <Input
              value={values.searchValue}
              onChange={(event) => updateValue('searchValue', event.target.value)}
              placeholder={t('management.my-properties.propertyWizardDialog.например_queen_s_res')}
              className={FIELD_CLASS}
            />
          </FieldShell>

          <FieldShell label={t('management.my-properties.propertyWizardDialog.название_объявления')} required>
            <Input
              value={values.title}
              onChange={(event) => updateValue('title', event.target.value)}
              placeholder={t('management.my-properties.propertyWizardDialog.например_3_к_квартир')}
              className={FIELD_CLASS}
            />
          </FieldShell>
        </section>
        ...
        {rules.supportsSummary && (
          <FieldShell label={t('management.my-properties.propertyWizardDialog.суть_проекта')} required>
            <WizardTextArea
              value={values.summary}
              onChange={(value) => updateValue('summary', value)}
              placeholder={t('management.my-properties.propertyWizardDialog.коротко_опишите_форм')}
              rows={3}
            />
          </FieldShell>
        )}

        <FieldShell label={t('management.my-properties.propertyWizardDialog.описание')} required>
          <WizardTextArea
            value={values.description}
            onChange={(value) => updateValue('description', value)}
            placeholder={t('management.my-properties.propertyWizardDialog.опишите_планировку_л')}
          />
        </FieldShell>

        <section className="grid gap-3 md:grid-cols-2">
          {rules.supportsRooms && (
            <FieldShell label={t('management.my-properties.propertyWizardDialog.количество_комнат')} required>
              <Input
                type="number"
                value={values.rooms}
          ...
          <FieldShell label={t('management.my-properties.propertyWizardDialog.площадь_м')} required>
            <Input
              type="number"
              value={values.area}
          ...
          <FieldShell label={t('management.my-properties.propertyWizardDialog.цена')} required={!values.priceOnRequest}>
            <Input
              type="number"
              value={values.price}
          ...
          <FieldShell label={t('management.my-properties.propertyWizardDialog.цена_за_м')} hint="Если пусто, посчитаем автоматически">
            <Input
              type="number"
        ...
        {rules.supportsPriceOnRequest && (
          <FieldShell label={t('management.my-properties.propertyWizardDialog.цена_по_запросу')} hint="Для проекта можно скрыть цену на витрине">
            <BinaryFlagField
      ...
      <div className="space-y-4">
        <section className="rounded-[20px] border border-[var(--hub-tile-icon-border)] bg-[var(--hub-action-hover)] p-3.5">
          <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
            <div className="text-[15px] font-normal text-[color:var(--workspace-text)]">{t('management.my-properties.propertyWizardDialog.адрес_и_геоточка')}</div>
            ...
            >
              ...
              {t('management.my-properties.propertyWizardDialog.выбрать_на_карте')}</Button>
          </div>

          <div className="mt-3 grid gap-3 xl:grid-cols-[minmax(0,1fr)_280px]">
            <div className="space-y-3">
              <FieldShell label={t('management.my-properties.propertyWizardDialog.адрес')} required hint="Укажите адрес">
                <WizardTextArea
                  value={values.address}
                  onChange={(value) => updateValue('address', value)}
                  placeholder={t('management.my-properties.propertyWizardDialog.ул_шерифа_химшиашвил')}
                  rows={3}
                />
              </FieldShell>
              ...
              <div className="rounded-[18px] border border-[var(--hub-tile-icon-border)] bg-[color-mix(in_srgb,var(--rail-bg)_82%,transparent)] px-3.5 py-3">
                  <div className="text-[11px] uppercase tracking-[0.14em] text-[color:var(--theme-accent-icon-dim)]">{t('management.my-properties.propertyWizardDialog.координаты')}</div>
                  <div className="mt-2 text-[13px] font-normal text-[color:var(--workspace-text)]">{mapCoordinatesLabel}</div>
              ...
              <div className="flex flex-col gap-2 sm:flex-row">
                ...
                >
                  <MapPinned className="size-4" />
                  {t('management.my-properties.propertyWizardDialog.выбрать_точку_на_кар')}</Button>
                ...
                >
                  ...
                  {t('management.my-properties.propertyWizardDialog.центр_города')}</Button>
              </div>
            </div>
            ...
            <div className="rounded-[18px] border border-[var(--hub-tile-icon-border)] bg-[linear-gradient(180deg,_rgba(10,31,24,0.78),_rgba(7,24,19,0.82))] p-3">
              ...
              >
                ...
                <div className="mt-3 text-[16px] font-normal tracking-[0.08em] text-[color:var(--workspace-text)]">{t('management.my-properties.propertyWizardDialog.место_под_карту')}</div>
                <div className="mt-2 text-[12px] text-[color:var(--hub-desc)]">{t('management.my-properties.propertyWizardDialog.нажмите_чтобы_открыт')}</div>
              </button>
              ...
              <div className="mt-4 rounded-[18px] border border-[color:var(--hub-tile-icon-border)] bg-[color-mix(in_srgb,var(--rail-bg)_80%,transparent)] p-3.5">
                <div className="text-[11px] uppercase tracking-[0.14em] text-[color:var(--theme-accent-icon-dim)]">{t('management.my-properties.propertyWizardDialog.выбранная_точка')}</div>
                <div className="mt-2 text-[14px] font-normal text-[color:var(--workspace-text)]">{mapLocationLabel}</div>
        ...
        {rules.supportsViews && (
          <FieldShell label={t('management.my-properties.propertyWizardDialog.вид')} hint="Можно выбрать несколько вариантов">
            <ChipGroup
        ...
        <section className={cn('grid gap-3', rules.supportsRoadType ? 'md:grid-cols-2' : 'md:grid-cols-1')}>
          {rules.supportsRoadType && (
            <FieldShell label={t('management.my-properties.propertyWizardDialog.дорога_к_объекту')}>
              <SingleChipGroup
          ...
          {rules.supportsShoreline && (
            <FieldShell label={t('management.my-properties.propertyWizardDialog.береговая_линия')} required>
              <SingleChipGroup
  ...
  function renderSpecsStep() {
    ...
    return (
      <div className="space-y-4">
        <section className="grid gap-3 md:grid-cols-2">
          {rules.supportsFloor && (
            <FieldShell label={t('management.my-properties.propertyWizardDialog.этаж')} required>
              <Input
                type="number"
                value={values.floor}
          ...
          {rules.supportsTotalFloors && (
            <FieldShell label={t('management.my-properties.propertyWizardDialog.этажность')} required>
              <Input
                type="number"
          ...
          {rules.supportsRenovation && (
            <FieldShell label={t('management.my-properties.propertyWizardDialog.ремонт')} required>
              <Select value={values.renovation} onValueChange={(value) => updateValue('renovation', value)}>
                <SelectTrigger className={SELECT_CLASS}>
                  <SelectValue placeholder={t('management.my-properties.propertyWizardDialog.выберите_ремонт')} />
                </SelectTrigger>
                <SelectContent>
          ...
          {rules.supportsCeilingHeight && (
            <FieldShell label={t('management.my-properties.propertyWizardDialog.высота_потолков')}>
              <Select value={values.ceilingHeight} onValueChange={(value) => updateValue('ceilingHeight', value)}>
                <SelectTrigger className={SELECT_CLASS}>
                  <SelectValue placeholder={t('management.my-properties.propertyWizardDialog.выберите_высоту')} />
                </SelectTrigger>
                <SelectContent>
                  {CEILING_HEIGHT_OPTIONS.map((option) => (
                    <SelectItem key={option} value={option}>
                      {option} {t('management.my-properties.propertyWizardDialog.м')}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </FieldShell>
        ...
        {rules.supportsBathrooms && (
          <section className="grid gap-3 md:grid-cols-2">
            <FieldShell label={t('management.my-properties.propertyWizardDialog.санузел')}>
              <SingleChipGroup
            ...
            <FieldShell label={t('management.my-properties.propertyWizardDialog.количество_санузлов')} required>
              <SingleChipGroup
        ...
        {rules.supportsBalcony && (
          <FieldShell label={t('management.my-properties.propertyWizardDialog.балкон_лоджия')}>
            <SingleChipGroup
        ...
        {(rules.supportsElevator || rules.supportsParking || rules.supportsPropertyUsage) && (
          <section className="grid gap-3 md:grid-cols-2">
            {rules.supportsElevator && (
              <FieldShell label={t('management.my-properties.propertyWizardDialog.лифт')}>
                <ChipGroup
            ...
            {rules.supportsParking && (
              <FieldShell label={t('management.my-properties.propertyWizardDialog.парковка')}>
                <ChipGroup
            ...
            {rules.supportsPropertyUsage && (
              <div className="md:col-span-2">
                <FieldShell label={t('management.my-properties.propertyWizardDialog.тип_недвижимости_наз')}>
                  <ChipGroup
        ...
        {(rules.supportsWallMaterial || rules.supportsGas || rules.supportsWaterSupply || rules.supportsSewage || rules.supportsLandType || rules.supportsElectricity) && (
          <section className="grid gap-3 md:grid-cols-2">
            {rules.supportsWallMaterial && (
              <FieldShell label={t('management.my-properties.propertyWizardDialog.материал_стен')}>
                <Select value={values.wallMaterial} onValueChange={(value) => updateValue('wallMaterial', value)}>
                  <SelectTrigger className={SELECT_CLASS}>
                    <SelectValue placeholder={t('management.my-properties.propertyWizardDialog.выберите_материал')} />
                  </SelectTrigger>
                  <SelectContent>
            ...
            {rules.supportsGas && (
              <FieldShell label={t('management.my-properties.propertyWizardDialog.газ')}>
                <BooleanChoiceField value={values.gas} onChange={(value) => updateValue('gas', value)} />
              </FieldShell>
            )}

            {rules.supportsWaterSupply && (
              <FieldShell label={t('management.my-properties.propertyWizardDialog.водоснабжение')}>
                <Select value={values.waterSupply} onValueChange={(value) => updateValue('waterSupply', value)}>
                  <SelectTrigger className={SELECT_CLASS}>
                    <SelectValue placeholder={t('management.my-properties.propertyWizardDialog.выберите_вариант')} />
                  </SelectTrigger>
                  <SelectContent>
            ...
            {rules.supportsSewage && (
              <FieldShell label={t('management.my-properties.propertyWizardDialog.канализация')}>
            ...
            {rules.supportsLandType && (
              <FieldShell label={t('management.my-properties.propertyWizardDialog.тип_участка')}>
                <Select value={values.landType} onValueChange={(value) => updateValue('landType', value)}>
                  <SelectTrigger className={SELECT_CLASS}>
                    <SelectValue placeholder={t('management.my-properties.propertyWizardDialog.выберите_тип')} />
                  </SelectTrigger>
                  <SelectContent>
            ...
            {rules.supportsElectricity && (
              <FieldShell label={t('management.my-properties.propertyWizardDialog.электричество')}>
        ...
        <section className={cn('grid gap-3', rules.supportsMediaUpload ? 'lg:grid-cols-3' : 'lg:grid-cols-2')}>
          <div className="space-y-3 lg:col-span-1">
            <FieldShell label={t('management.my-properties.propertyWizardDialog.главное_фото')} hint="Используется в карточке и таблице">
              <FileField
                title={t('management.my-properties.propertyWizardDialog.загрузить_фото')}
          ...
          <div className="space-y-3 lg:col-span-1">
            <FieldShell label={t('management.my-properties.propertyWizardDialog.планировка')}>
              <FileField
                title={t('management.my-properties.propertyWizardDialog.загрузить_планировку')}
          ...
          {rules.supportsMediaUpload && (
            <div className="space-y-3 lg:col-span-1">
              <FieldShell label={t('management.my-properties.propertyWizardDialog.медиаконтент')} hint="Особенно актуально для проектов.">
                <FileField
                  title={t('management.my-properties.propertyWizardDialog.загрузите_дополнител')}
  ...
  function renderDealStep() {
    ...
    return (
      <div className="space-y-4">
        <FieldShell label={t('management.my-properties.propertyWizardDialog.инфраструктура_и_пре')} hint="Сильные стороны объекта для продажи и презентации">
          <ChipGroup
        ...
        <section className="grid gap-3 md:grid-cols-2">
          <FieldShell label={t('management.my-properties.propertyWizardDialog.размер_комиссии')} required>
            <div className="group/commission space-y-3">
              <Select value={values.commissionPercent} onValueChange={(value) => updateValue('commissionPercent', value)}>
                <SelectTrigger className={SELECT_CLASS}>
                  <SelectValue placeholder={t('management.my-properties.propertyWizardDialog.выберите_комиссию')} />
                </SelectTrigger>
                <SelectContent>
              ...
              <div className="text-[12px] text-[color:var(--hub-stat-label)]">
                {t('management.my-properties.propertyWizardDialog.наведите_чтобы_увиде')}</div>
              ...
              <div className="pointer-events-none max-h-0 overflow-hidden rounded-[18px] border border-[var(--hub-tile-icon-border)] bg-[rgba(255,255,255,0.04)] opacity-0 transition-all duration-200 group-hover/commission:pointer-events-auto group-hover/commission:max-h-[420px] group-hover/commission:p-4 group-hover/commission:opacity-100 group-focus-within/commission:pointer-events-auto group-focus-within/commission:max-h-[420px] group-focus-within/commission:p-4 group-focus-within/commission:opacity-100">
                <div className="text-[12px] font-normal text-[color:var(--workspace-text)]">{t('management.my-properties.propertyWizardDialog.рекомендации_по_выбо')}</div>
                <div className="mt-3 space-y-2 text-[13px] leading-relaxed text-[color:var(--app-text-muted)]">
                  {COMMISSION_OPTIONS.map((option) => (
                    <div key={option.value}>
                      <span className="font-normal text-[color:var(--workspace-text)]">{option.label}</span>
          ...
          <FieldShell label={t('management.my-properties.propertyWizardDialog.продавец')} required>
        ...
        <section className="grid gap-3 md:grid-cols-2">
          <FieldShell label={t('management.my-properties.propertyWizardDialog.ипотека')}>
            <BinaryFlagField
          ...
          <FieldShell label={t('management.my-properties.propertyWizardDialog.рассрочка')}>
            <BinaryFlagField
        ...
        <div className="relative h-full overflow-hidden rounded-[28px] border border-[var(--hub-card-border)] bg-[radial-gradient(circle_at_top,_rgba(16,185,129,0.12),_transparent_42%),linear-gradient(180deg,_rgba(9,36,28,0.985),_rgba(6,20,16,0.98))] text-[color:var(--workspace-text)] shadow-[0_30px_120px_rgba(0,0,0,0.45)]">
          <div className="grid h-full lg:grid-cols-[212px_minmax(0,1fr)] xl:grid-cols-[212px_minmax(0,1fr)_272px]">
            <aside className="flex min-h-0 flex-col border-b border-[color:var(--hub-tile-icon-border)] bg-[linear-gradient(180deg,_rgba(8,30,23,0.94),_rgba(7,24,19,0.9))] p-3 lg:border-r lg:border-b-0 xl:p-3.5">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <h2 className="text-[17px] font-normal leading-[1.05] text-[color:var(--workspace-text)]">{pageTitle}</h2>
                  <p className="mt-1 text-[12px] leading-snug text-[color:var(--hub-desc)]">
                    {t('management.my-properties.propertyWizardDialog.пошаговая_сборка_кар')}</p>
                </div>
                <button
                  type="button"
                  onClick={onClose}
                ...
                {PROPERTY_WIZARD_STEPS.map((step, index) => {
                  ...
                  return (
                    ...
                    >
                        <div
                          className={cn(
                          'flex size-7 shrink-0 items-center justify-center rounded-full border text-[10px] font-normal',
                          active
                       ...
                       <div className="min-w-0 flex-1">
                         <div className="flex min-w-0 items-center gap-1.5">
                           ...
                           <span className="truncate text-[13px] font-normal leading-snug text-[color:var(--workspace-text)]">{step.title}</span>
                         </div>
                       </div>
                     </button>
                  )
            ...
            <section className="flex min-h-0 flex-col">
              <div className="border-b border-[color:var(--hub-tile-icon-border)] px-4 py-3 xl:px-5 xl:py-3.5">
                <DialogHeader className="text-left">
                  <div className="inline-flex w-fit items-center rounded-full border border-emerald-400/20 bg-emerald-400/10 px-2.5 py-0.5 text-[11px] font-normal uppercase tracking-[0.14em] text-emerald-300">
                    {stepBadgeLabel}
                  </div>
                ...
                <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
                  <div className="flex flex-col gap-2.5 sm:flex-row">
                    <Button
                      ...
                      className="h-10 rounded-full border-[var(--hub-card-border)] bg-transparent px-4 text-[12px] text-[var(--theme-accent-link-dim)] hover:bg-[var(--hub-action-hover)] hover:text-[color:var(--workspace-text)]"
                    >
                      {t('management.my-properties.propertyWizardDialog.отмена')}</Button>
                    ...
                    {mode === 'create' && (
                      ...
                      >
                        ...
                        {t('management.my-properties.propertyWizardDialog.сохранить_черновик')}</Button>
                    )}
                  </div>
                  ...
                  <div className="flex flex-col gap-2.5 sm:flex-row">
                    ...
                    >
                      ...
                      {t('management.my-properties.propertyWizardDialog.шаг_назад')}</Button>
                    ...
                    >
                      {activeStepIndex < PROPERTY_WIZARD_STEPS.length - 1 ? (
                        <>
                          {t('management.my-properties.propertyWizardDialog.далее')}<ChevronRight className="size-4" />
                        </>
                      ) : (
                        <>
            ...
            <aside className="hidden min-h-0 flex-col border-l border-[color:var(--hub-tile-icon-border)] bg-[linear-gradient(180deg,_rgba(10,31,24,0.9),_rgba(7,23,18,0.94))] p-4 xl:flex">
              <div>
                ...
                <h3 className="mt-1.5 text-[18px] font-normal leading-[1.05] text-[color:var(--workspace-text)]">{t('management.my-properties.propertyWizardDialog.черновик_объявления')}</h3>
              </div>
              ...
              <div className="mt-3 flex-1 overflow-y-auto">
                <div className="rounded-[20px] border border-[var(--hub-tile-icon-border)] bg-[var(--hub-action-hover)] p-3.5">
                  ...
                  <div className="mt-3 text-[22px] font-normal leading-none text-emerald-300">{previewPriceLabel}</div>
                  <div className="mt-1 text-[11px] leading-snug text-[color:var(--hub-desc)]">{previewPricePerM2Label}</div>

                  <div className="mt-3 line-clamp-3 text-[16px] font-normal leading-snug text-[color:var(--workspace-text)]">
                  ...
                  <div className="mt-3 grid grid-cols-3 gap-1.5 border-t border-[var(--hub-action-hover)] pt-2.5">
                    {previewFacts.map((item) => (
                      <div key={item.label} className="min-w-0 rounded-[14px] border border-[var(--hub-action-hover)] bg-[color-mix(in_srgb,var(--rail-bg)_80%,transparent)] p-2">
                        ...
                        <div className="mt-1 truncate text-[12px] font-normal text-[color:var(--workspace-text)]">{item.value}</div>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            <div className="absolute inset-0 z-50 flex items-center justify-center bg-[rgba(3,10,8,0.72)] p-4 backdrop-blur-sm">
              <div className="flex h-full max-h-[760px] w-full max-w-[980px] flex-col overflow-hidden rounded-[30px] border border-[var(--hub-card-border)] bg-[linear-gradient(180deg,_rgba(9,34,27,0.98),_rgba(6,20,16,0.98))] shadow-[0_28px_80px_rgba(0,0,0,0.5)]">
                <div className="flex items-start justify-between gap-4 border-b border-[color:var(--hub-tile-icon-border)] px-5 py-4">
                  <div>
                    <div className="text-[18px] font-normal text-[color:var(--workspace-text)]">{t('management.my-properties.propertyWizardDialog.карта_локации')}</div>
                    <div className="mt-1 text-[13px] text-[color:var(--hub-desc)]">
                      {t('management.my-properties.propertyWizardDialog.открой_карту_и_поста')}</div>
                  </div>
                ...
                <div className="grid min-h-0 flex-1 gap-0 lg:grid-cols-[280px_minmax(0,1fr)]">
                  <div className="border-b border-[color:var(--hub-tile-icon-border)] bg-[color-mix(in_srgb,var(--rail-bg)_70%,transparent)] p-5 lg:border-r lg:border-b-0">
                    <div className="rounded-[20px] border border-[var(--hub-tile-icon-border)] bg-[color-mix(in_srgb,var(--rail-bg)_82%,transparent)] p-4">
                      <div className="text-[11px] uppercase tracking-[0.14em] text-[color:var(--theme-accent-icon-dim)]">{t('management.my-properties.propertyWizardDialog.текущая_точка')}</div>
                      <div className="mt-2 text-[15px] font-normal text-[color:var(--workspace-text)]">{mapLocationLabel}</div>
                      <div className="mt-1 text-[12px] text-[color:var(--hub-desc)]">{mapCoordinatesLabel}</div>
                    </div>
                    <div className="mt-4 grid gap-2">
                      ...
                      >
                        <Crosshair className="size-4" />
                        {t('management.my-properties.propertyWizardDialog.поставить_в_центр_го')}</Button>
                      ...
                      >
                        {t('management.my-properties.propertyWizardDialog.готово')}</Button>
                    </div>
                  </div>
                  ...
                  <div className="min-h-0 p-5">
                    ...
                    >
                      ...
                      <div className="absolute right-4 top-4 rounded-full border border-[var(--hub-tile-icon-border)] bg-[color-mix(in_srgb,var(--rail-bg)_88%,transparent)] px-3 py-1 text-[11px] text-[color:var(--hub-stat-label)]">
                        {t('management.my-properties.propertyWizardDialog.клик_по_карте_ставит')}</div>
                      <div className="pointer-events-none absolute inset-0 flex items-center justify-center">
                        <div className="rounded-[24px] border border-dashed border-[color:var(--hub-card-border)] bg-[color-mix(in srgb, var(--rail-bg) 85%, transparent)] px-8 py-5 text-center shadow-[0_20px_60px_rgba(0,0,0,0.2)]">
                          <div className="text-[22px] font-normal tracking-[0.08em] text-[color:var(--workspace-text)]">{t('management.my-properties.propertyWizardDialog.место_под_карту')}</div>
                        </div>
                      </div>
                      {selectedMapPoint && (
                        <div
