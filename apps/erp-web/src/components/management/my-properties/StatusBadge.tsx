...
export function SaleStatusBadge({ status }: { status: SaleStatus }) {
  ...
  return (
    <span className={cn(
      'inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-normal whitespace-nowrap',
      cfg.className,
    )}>
      {cfg.label}
    </span>
...
export function ConditionBadge({ state }: { state: ConditionState }) {
  ...
  return (
    <span className={cn(
      'inline-flex items-center gap-1 rounded-full border px-2.5 py-0.5 text-xs font-normal whitespace-nowrap',
      cfg.className,
    )}>
      <span>{cfg.icon}</span>
      {cfg.label}
...
// ─── Метка MLS ────────────────────────────────────────────────────────────────
// Объект размещён в MLS-круге: комиссия гарантирована, виден всей сети агентов.

export function MlsBadge() {
  return (
    <span
      className={cn(
        'inline-flex items-center rounded px-2 py-1 text-base font-normal whitespace-nowrap',
        'text-[color:var(--gold)] bg-[color-mix(in_srgb,var(--gold)_14%,transparent)]',
        'shadow-[inset_0_0_0_1px_color-mix(in_srgb,var(--gold)_45%,transparent)]',
      )}
    >
      MLS
    </span>
