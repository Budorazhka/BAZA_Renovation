...
function CheckboxGroup<T extends string>({
  ...
  return (
    <div className="space-y-2">
      <p className="text-xs font-normal uppercase tracking-wide text-[color:var(--app-text-subtle)]">{label}</p>
...
function RangeInputs({
  ...
  return (
    <div className="space-y-2">
      <p className="text-xs font-normal uppercase tracking-wide text-[color:var(--app-text-subtle)]">
        {label}, {unit}
      </p>
      <div className="flex items-center gap-2">
        <input
          type="number"
          placeholder={t('management.my-properties.filterPanel.от')}
          value={minValue}
        ...
        <input
          type="number"
          placeholder={t('management.my-properties.filterPanel.до')}
          value={maxValue}
...
export function FilterPanel({ open, onClose, filters, onApply }: FilterPanelProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      >
        <DialogHeader>
          <DialogTitle className="text-[color:var(--app-text)]">{t('management.my-properties.filterPanel.фильтры')}</DialogTitle>
        </DialogHeader>

        <div className="space-y-5 py-1">
          <CheckboxGroup<PropertyType>
            label={t('management.my-properties.filterPanel.тип_объекта')}
          ...
          <CheckboxGroup<SaleStatus>
            label={t('management.my-properties.filterPanel.статус_продажи')}
            items={SALE_STATUSES}
          ...
          <CheckboxGroup<ConditionState>
            label={t('management.my-properties.filterPanel.состояние')}
            items={CONDITIONS}
          ...
          <RangeInputs
            label={t('management.my-properties.filterPanel.цена')}
            unit="$"
          ...
          <RangeInputs
            label={t('management.my-properties.filterPanel.площадь')}
            unit="м²"
        ...
        <div className="flex items-center justify-between gap-3 border-t border-[var(--green-border)] pt-4">
          ...
          >
            {t('management.my-properties.filterPanel.сбросить')}</button>
          <button type="button" onClick={handleApply} className="alphabase-section-primary !normal-case px-6 py-2">
            {t('management.my-properties.filterPanel.применить')}</button>
        </div>
      </DialogContent>
    </Dialog>
  )
