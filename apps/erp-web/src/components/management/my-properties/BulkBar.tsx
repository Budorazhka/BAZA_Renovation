...
export function BulkBar({ selectedCount, onApply, onDeleteSelected }: BulkBarProps) {
  ...
  if (!hasSelection) {
    return (
      <div className="flex items-center rounded-xl border border-[var(--green-border)] bg-[var(--green-deep)]/80 px-4 py-2.5 text-sm text-[color:var(--app-text-subtle)]">
        {t('management.my-properties.bulkBar.отметьте_объекты_для')}</div>
    )
  }
  ...
  return (
    <div className="flex flex-wrap items-center gap-3 rounded-xl border border-[color:var(--hub-card-border)] bg-[var(--green-deep)] px-4 py-2.5 text-sm shadow-[inset_0_0_0_1px_rgba(201,168,76,0.06)]">
      <p className="font-normal text-[color:var(--workspace-text)] min-w-0">
        {t('management.my-properties.bulkBar.выбрано')}{selectedCount} {selectedCount === 1 ? 'объект' : selectedCount < 5 ? 'объекта' : 'объектов'}
      </p>

      <div className="flex items-center gap-2 flex-wrap ml-auto">
        <span className="text-xs text-[color:var(--hub-stat-label)]">{t('management.my-properties.bulkBar.назначить_статус')}</span>
        ...
        >
          ...
          {t('management.my-properties.bulkBar.в_продаже')}</button>
        ...
        >
          ...
          {t('management.my-properties.bulkBar.забронировано')}</button>
        ...
        >
          ...
          {t('management.my-properties.bulkBar.продано')}</button>
        ...
        >
          ...
          {t('management.my-properties.bulkBar.архивировать')}</button>
        ...
        >
          ...
          {t('management.my-properties.bulkBar.удалить')}</button>
      </div>
    </div>
  )
}
