  ...
  return (
    <div className="rounded-xl border border-[color:var(--gold)]/15 bg-[#0a1f1a] px-4 py-3 shadow-[inset_0_0_0_1px_rgba(201,168,76,0.06)]">
      <div className="flex items-center gap-2 flex-wrap">
        <span className="mr-1 shrink-0 text-xs font-normal uppercase tracking-wide text-[color:var(--theme-accent-heading)]/80">
          {t('management.my-properties.propertyAlerts.обзор')}</span>
        {chips.map((chip) => {
        ...
        {activeFilter && (
          ...
          >
            {t('management.my-properties.propertyAlerts.сбросить')}</button>
        )}
      </div>
    </div>
  )
