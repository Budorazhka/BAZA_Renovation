...
interface DynamicKpiCardsProps {
    ...
    labels?: Partial<Record<keyof DynamicKpi, string>>;
}
...
export function DynamicKpiCards({ data, todayData, periodLabel, variant = "full", labels }: DynamicKpiCardsProps) {
    const { t } = useI18n();
    const kpiConfig =
        variant === "directOnly"
    ...
    return (
        <div className="space-y-3">
            <div className="flex items-center gap-2">
                <h3 className="text-sm font-normal text-slate-800">{t('analytics-network.dynamic-kpi-cards.период')}</h3>
                <Badge variant="secondary" className="text-sm font-normal text-slate-900">
                    {periodLabel}
                </Badge>
            </div>
                        ...
                        <Card key={kpi.key} className="p-4">
                            <CardContent className="flex min-h-[136px] flex-col gap-2 p-0">
                                <div className="flex items-center justify-between">
                                    <Badge className="max-w-[76%] bg-slate-100 text-slate-700 shadow-none text-sm font-normal border border-slate-200">
                                        +{todayValue.toLocaleString("ru-RU")} {t('analytics-network.dynamic-kpi-cards.за_сегодня')}</Badge>
                                    <div className={`rounded-md p-2 ${kpi.iconBg}`}>
                                        <Icon className={`h-4 w-4 ${kpi.iconColor}`} />
                                    </div>
                                </div>
                                <div className="mt-1 text-center">
                                    <p className="text-xl font-normal leading-none text-slate-900 tabular-nums">
                                        {value.toLocaleString("ru-RU")}
                                    </p>
                                    <p className="mt-1 text-base font-normal leading-snug text-slate-800 break-words">
                                        {labels?.[kpi.key] ?? kpi.label}
                                    </p>
                                    <p className="text-sm font-medium text-slate-700">{t('analytics-network.dynamic-kpi-cards.за')}{periodLabelLower}</p>
                                </div>
                            </CardContent>
                        </Card>
                    );
