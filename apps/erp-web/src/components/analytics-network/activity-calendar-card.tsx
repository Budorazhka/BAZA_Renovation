        ...
        <Card className={cn(className)}>
            <CardHeader className="pb-2">
                <div className="flex flex-col items-center gap-1 text-center">
                    <CardTitle className={cn("text-center text-xl font-normal text-slate-900 sm:text-2xl", highContrast && "text-2xl sm:text-3xl")}>
                        {t('analytics-network.activity-calendar-card.календарь_активности')}</CardTitle>
                ...
                {period !== "allTime" ? (
                    <div className="grid grid-cols-7 gap-1 sm:gap-1.5">
                        {weekDayLabels.map((dayLabel) => (
                            <div key={dayLabel} className={cn("pb-2 text-center text-base font-normal text-slate-800", highContrast && "text-lg text-slate-900")}>
                                {dayLabel}
                            </div>
                        ))}
                            ...
                            return (
                                ...
                                >
                                    <div className="flex items-center justify-between mb-2">
                                        <span className="text-2xl font-normal leading-none opacity-100 text-slate-900">
                                            {entry.dateNumber}
                                        </span>
                                        <span className="text-lg font-normal opacity-100 text-slate-900">
                                            {entry.total}
                                        </span>
                                    </div>
                        ...
                        {allTimeEntries.map((entry) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <p className={cn("mt-1 text-xs font-medium", highContrast && "text-sm font-normal")}>{entry.total.toLocaleString("ru-RU")}</p>
                                </button>
                            );
                        })}
                    </div>
                ...
                <div className={cn("flex flex-wrap items-center gap-x-4 gap-y-2 text-base text-slate-800", highContrast && "text-lg text-slate-900")}>
                    {period === "week" && (
                        <span className="inline-flex items-center gap-2">
                            ...
                            {t('analytics-network.activity-calendar-card.выделенная_неделя')}</span>
                    )}
                    <span className="inline-flex items-center gap-2">
                        <span className="h-3 w-3 rounded-full bg-slate-300 border border-slate-200" />
                        {t('analytics-network.activity-calendar-card.0_активностей')}</span>
                    <span className="inline-flex items-center gap-2">
                        <span className="h-3 w-3 rounded-full bg-amber-400" />
                        {t('analytics-network.activity-calendar-card.1_4_активности')}</span>
                    <span className="inline-flex items-center gap-2">
                        <span className="h-3 w-3 rounded-full bg-emerald-400" />
                        {t('analytics-network.activity-calendar-card.5_активностей')}</span>
                    <span className="inline-flex items-center gap-2">
                        <span className="h-3 w-3 rounded-full bg-slate-400" />
                        {t('analytics-network.activity-calendar-card.прошедшие_дни')}</span>
                </div>

                {selectedEntry && (
                    <div className="rounded-xl border border-slate-300 bg-slate-50 p-4">
                        <p className={cn("text-lg font-normal text-slate-900", highContrast && "text-xl")}>{selectedEntry.labelLong}</p>
                        <div className={cn("mt-2 flex flex-wrap items-center gap-4 text-base text-slate-800", highContrast && "text-lg text-slate-900")}>
                            <span className="font-normal">{t('analytics-network.activity-calendar-card.всего')}{selectedEntry.total.toLocaleString("ru-RU")}</span>
                            <span className="font-normal">{t('analytics-network.activity-calendar-card.звонки')}{selectedEntry.calls.toLocaleString("ru-RU")}</span>
                            <span className="font-normal">{t('analytics-network.activity-calendar-card.чаты')}{selectedEntry.chats.toLocaleString("ru-RU")}</span>
                            <span className="font-normal">{t('analytics-network.activity-calendar-card.рассылки')}{selectedEntry.selections.toLocaleString("ru-RU")}</span>
                        </div>
                    </div>
                )}
            </CardContent>
                ...
                <DialogContent className="sm:max-w-lg">
                    ...
                    {selectedEntry ? (
                        <div className="grid gap-4 sm:grid-cols-3">
                            <div className="rounded-xl border border-slate-300 bg-slate-50 p-4">
                                <p className="text-base font-normal text-slate-900">{t('analytics-network.activity-calendar-card.звонки')}</p>
                                <p className="text-2xl font-normal text-slate-900">{selectedEntry.calls.toLocaleString("ru-RU")}</p>
                            ...
                            <div className="rounded-xl border border-slate-300 bg-slate-50 p-4">
                                <p className="text-base font-normal text-slate-900">{t('analytics-network.activity-calendar-card.рассылки')}</p>
                            ...
                            <div className="rounded-xl border border-slate-300 bg-slate-50 p-4">
                                <p className="text-base font-normal text-slate-900">{t('analytics-network.activity-calendar-card.чаты')}</p>
                                <p className="text-2xl font-normal text-slate-900">{selectedEntry.chats.toLocaleString("ru-RU")}</p>
                                <p className="text-base font-normal text-slate-800">
                                    {selectedEntry.total > 0
