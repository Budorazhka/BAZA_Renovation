...
export function CabinetSwitcher({
  ...
  return (
    <div className={cn("flex w-full flex-col gap-1.5", className)}>
      <p className="text-[0.8125rem] font-medium text-[color:var(--app-text-muted)]">{t('analytics-network.cabinet-switcher.активный_кабинет')}</p>
      <div className="flex w-full flex-col gap-2 sm:flex-row sm:items-center">
        <Select value={value} onValueChange={onValueChange}>
          <SelectTrigger className="h-9 w-full sm:max-w-[320px]">
            <SelectValue placeholder={t('analytics-network.cabinet-switcher.выберите_кабинет')} />
          </SelectTrigger>
          <SelectContent>
            {options.map((option) => (
