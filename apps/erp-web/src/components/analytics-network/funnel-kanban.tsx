...
function getColumnTone({
    ...
    if (columnId === "in_progress" || columnId === "preparation" || columnId === "active") {
        return {
            pill: "bg-[#245d66]/24 text-[#9bcbd1]",
            bar: "bg-[#24a6b8]",
        };
    }

    return {
        pill: "bg-[#0f3d2a]/35 text-[#aebeb5]",
        bar: "bg-[#56a879]",
    };
}
        ...
        <DropdownMenu>
            <DropdownMenuTrigger asChild>
                ...
                >
                    ...
                    {t('analytics-network.funnel-kanban.конверсии')}</Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent
                align="end"
    ...
    if (funnels.length === 0) {
        return (
            <Card className={cn(isLeads && leadsCardClass)}>
                <CardHeader>
                    <CardTitle className={cn(isLeads && "text-[color:var(--app-text)]")}>{t('analytics-network.funnel-kanban.воронки')}</CardTitle>
                    <CardDescription className={cn(isLeads && "text-[#e8dcc4]")}>{t('analytics-network.funnel-kanban.нет_данных_для_отобр')}</CardDescription>
                </CardHeader>
            </Card>
        );
    }
                ...
                <CardHeader className="gap-3">
                    <div className="space-y-1 text-center">
                        <CardTitle className={cn("text-base font-medium sm:text-lg", isLeads && "text-[color:var(--app-text)]")}>
                            {t('analytics-network.funnel-kanban.воронки_в_канбане')}</CardTitle>
                        <CardDescription className={cn(isLeads && "text-[#e8dcc4]")}>
                            {t('analytics-network.funnel-kanban.все_этапы_отражены_п')}</CardDescription>
                    </div>
                    <TabsList
                        className={cn(
                    ...
                    {funnels.map((board) => (
                        <TabsContent key={board.id} value={board.id} className="m-0 space-y-3">
                            <div className="flex flex-wrap items-center gap-2">
                                ...
                                >
                                    {t('analytics-network.funnel-kanban.всего')}{board.totalCount.toLocaleString("ru-RU")}
                                </Badge>
                                <Badge
                                    variant="secondary"
                                    className={cn(isLeads && "border-[rgba(229,196,136,0.3)] bg-[color-mix(in_srgb,var(--gold-dark)_45%,transparent)] text-[#e8dcc4]")}
                                >
                                    {t('analytics-network.funnel-kanban.в_работе')}{board.activeCount.toLocaleString("ru-RU")}
                                </Badge>
                                <Badge
                                    variant="secondary"
                                    className={cn(isLeads && "border-[rgba(229,196,136,0.3)] bg-[color-mix(in_srgb,var(--gold-dark)_45%,transparent)] text-[#e8dcc4]")}
                                >
                                    {t('analytics-network.funnel-kanban.отказ')}{board.rejectionCount.toLocaleString("ru-RU")}
                                </Badge>
                                {board.closedCount > 0 && (
                                    <Badge
                                        variant="secondary"
                                        className={cn(isLeads && "border-[rgba(229,196,136,0.3)] bg-[color-mix(in_srgb,var(--gold-dark)_45%,transparent)] text-[#e8dcc4]")}
                                    >
                                        {t('analytics-network.funnel-kanban.закрыто')}{board.closedCount.toLocaleString("ru-RU")}
                                    </Badge>
                                )}
...
function FunnelColumnCard({
    ...
    const displayStages = [...column.stages].reverse();
                            ...
                            <div className="flex min-w-0 items-start justify-between gap-2">
                                <div className="min-w-0">
                                    <p className={cn("text-[11px]", isLeads ? "text-[#e8dcc4]/80" : "text-muted-foreground")}>
                                        {t('analytics-network.funnel-kanban.этап')}{stage.order}
                                    </p>
