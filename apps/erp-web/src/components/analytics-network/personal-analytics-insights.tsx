    ...
    return (
        <div className="grid min-w-0 gap-4 xl:grid-cols-2 xl:items-stretch">
                ...
                <CardHeader className="pb-2">
                    <div className="space-y-2">
                        <div className="space-y-1 text-center">
                            <CardTitle className="text-2xl font-normal text-slate-900 sm:text-3xl">{"\u041f\u043b\u0430\u043d/\u0444\u0430\u043a\u0442"}</CardTitle>
                            <p className="text-base text-slate-700">
                                {activePlanBucket === "week" ? "\u041f\u043b\u0430\u043d \u043d\u0430 \u043d\u0435\u0434\u0435\u043b\u044e" : "\u041f\u043b\u0430\u043d \u043d\u0430 \u043c\u0435\u0441\u044f\u0446"}
                            </p>
                        </div>
                        <div className="flex flex-wrap items-center justify-center gap-2 sm:justify-end">
                            <span className="rounded-md border bg-muted/30 px-2.5 py-1 text-lg font-normal text-slate-900">
                            ...
                            {allowPlanEditing && (
                                ...
                                >
                                    {t('analytics-network.personal-analytics-insights.установить_план')}</Button>
                            )}
                        </div>
                    </div>
                </CardHeader>
                <CardContent className="flex flex-1 flex-col justify-between gap-4 pt-1">
                        ...
                        {planRows.map((row) => {
                            ...
                            return (
                                <div key={row.key} className="rounded-md border bg-muted/20 p-3">
                                    ...
                                    <p className="mt-1 break-words text-3xl font-normal leading-none text-slate-900">
                    ...
                    <div className="grid gap-3 sm:grid-cols-3">
                        <div className="rounded-md border bg-muted/20 px-3 py-2.5">
                            <p className="text-sm font-medium text-slate-700">{t('analytics-network.personal-analytics-insights.выполнено_метрик')}</p>
                            <p className="text-3xl font-normal leading-none text-slate-900 tabular-nums">
                                {completedMetricsCount}/{planRows.length}
                            </p>
                        </div>
                        <div className="rounded-md border bg-muted/20 px-3 py-2.5">
                            <p className="text-sm font-medium text-slate-700">{t('analytics-network.personal-analytics-insights.осталось_до_плана')}</p>
                            <p className="text-3xl font-normal leading-none text-slate-900 tabular-nums">{totalRemaining.toLocaleString("ru-RU")}</p>
                        </div>
                        <div className="rounded-md border bg-muted/20 px-3 py-2.5">
                            <p className="text-sm font-medium text-slate-700">{t('analytics-network.personal-analytics-insights.сверх_плана')}</p>
                            <p className="text-3xl font-normal leading-none text-slate-900 tabular-nums">{totalSurplus.toLocaleString("ru-RU")}</p>
                        </div>
                    </div>

                    <div className="space-y-2 rounded-md border bg-muted/15 px-3 py-3">
                        <p className="text-base font-medium text-slate-700">{t('analytics-network.personal-analytics-insights.что_добрать_до_плана')}</p>
                        {planGapRows.map((row) => (
                            <div key={`${row.key}-gap`} className="flex items-center justify-between gap-2 text-base">
                                ...
                                {row.remaining > 0 ? (
                                    <span className="font-normal text-amber-700">+{row.remaining.toLocaleString("ru-RU")}</span>
                                ) : (
                                    <span className="font-normal text-emerald-700">{t('analytics-network.personal-analytics-insights.выполнено')}</span>
                                )}
                            </div>
                        ))}
                    </div>
            ...
            {allowPlanEditing && (
                <Dialog open={isPlanDialogOpen} onOpenChange={setIsPlanDialogOpen}>
                    <DialogContent className="sm:max-w-md">
                        <DialogHeader>
                            <DialogTitle>{t('analytics-network.personal-analytics-insights.установить_план')}</DialogTitle>
                            <DialogDescription>{t('analytics-network.personal-analytics-insights.задайте_цели_для_нед')}</DialogDescription>
                        </DialogHeader>

                        <Tabs value={editBucket} onValueChange={(v) => setEditBucket(v as PlanBucket)}>
                            <TabsList className="grid w-full grid-cols-2">
                                <TabsTrigger value="week">{t('analytics-network.personal-analytics-insights.неделя')}</TabsTrigger>
                                <TabsTrigger value="month">{t('analytics-network.personal-analytics-insights.месяц')}</TabsTrigger>
                            </TabsList>
                        ...
                        <DialogFooter>
                            <Button variant="outline" onClick={() => setIsPlanDialogOpen(false)}>
                                {t('analytics-network.personal-analytics-insights.отмена')}</Button>
                            <Button
                                onClick={() => {
                            ...
                            >
                                {t('analytics-network.personal-analytics-insights.сохранить')}</Button>
                        </DialogFooter>
                    </DialogContent>
                </Dialog>
            )}
            ...
            <Card className="xl:col-span-2">
                <CardHeader className="pb-2">
                    <CardTitle className="text-center text-base font-medium sm:text-lg">{t('analytics-network.personal-analytics-insights.статусы_лидов_в_рабо')}</CardTitle>
                </CardHeader>
                    ...
                    {totalCount > 0 && (
                        <div className="space-y-2">
                            ...
                            <p className="text-[11px] text-muted-foreground">
                                {t('analytics-network.personal-analytics-insights.всего_лидов_в_воронк')}{totalCount.toLocaleString("ru-RU")}
                            </p>
                        </div>
                    )}
                        ...
                        {columnRows.map((row) => (
                            <div key={row.id} className="rounded-md border bg-muted/20 px-3 py-2">
                                ...
                                <p className="mt-1 text-[11px] text-muted-foreground">
                                    {row.count.toLocaleString("ru-RU")} {t('analytics-network.personal-analytics-insights.лидов')}{statusHintByColumn[row.id] ?? "этап воронки"}
                                </p>
                            </div>
                        ))}
                    </div>
                        ...
                        <div className="space-y-2 rounded-lg border bg-muted/25 p-3">
                            <div className="grid gap-2 sm:grid-cols-2">
                                <div className="rounded-md border bg-background/70 px-3 py-2.5">
                                    <p className="text-xs text-muted-foreground">
                                        <HintLabel
                                            label={t('analytics-network.personal-analytics-insights.колонок_в_воронке')}
                                ...
                                <div className="rounded-md border bg-background/70 px-3 py-2.5">
                                    <p className="text-xs text-muted-foreground">
                                        <HintLabel
                                            label={t('analytics-network.personal-analytics-insights.средняя_нагрузка_на')}
