...
export function SupremeOwnerDashboardPage() {
      ...
      <div className="w-full space-y-4 px-3 py-4 sm:px-5">
        <Card>
          <CardContent className="p-6 text-center">
            <p className="text-base font-medium">{t('owner.supremeOwnerDashboardPage.кабинет_недоступен')}</p>
            <p className="text-sm text-muted-foreground">
              {t('owner.supremeOwnerDashboardPage.не_удалось_загрузить')}</p>
          </CardContent>
        </Card>
      </div>
    )
  ...
  return (
    <div className="network-analytics-theme agency-analytics-theme w-full max-w-full space-y-4 overflow-x-hidden px-3 py-4 sm:px-5">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <div className="space-y-1">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <h1 className="text-readable-2xl font-medium text-high-contrast">{t('owner.supremeOwnerDashboardPage.дашборд_сети')}</h1>
        ...
        <div className="flex shrink-0 flex-wrap items-center justify-end gap-2">
          ...
          >
            ...
            {t('owner.supremeOwnerDashboardPage.вернуться_на_главный')}</Button>
      ...
      {selectedCabinet?.scope === "partner" && (
        <div className="flex flex-wrap items-center gap-2">
          ...
          >
            {t('owner.supremeOwnerDashboardPage.аналитика')}{selectedCabinet.label}
          </Button>
          <Button
          ...
          >
            {t('owner.supremeOwnerDashboardPage.сеть')}{selectedCabinet.label}
          </Button>
        </div>
      )}
      ...
      <div className="grid items-stretch gap-5 lg:grid-cols-[380px_1fr]">
        <div className="flex min-w-0 flex-col gap-4">
          ...
          <MailingsInbox
            ...
            variant="cabinet"
            title={t('owner.supremeOwnerDashboardPage.оповещения')}
            description={t('owner.supremeOwnerDashboardPage.рассылки_в_личный_ка')}
          />
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-readable-sm text-high-contrast">{t('owner.supremeOwnerDashboardPage.партн_ры')}</CardTitle>
            </CardHeader>
            <CardContent className="space-y-3 pt-1">
              <Input
                value={partnerListSearch}
                onChange={(e) => setPartnerListSearch(e.target.value)}
                placeholder={t('owner.supremeOwnerDashboardPage.поиск_по_имени')}
              ...
              <div className="flex flex-wrap gap-2">
                ...
                >
                  {t('owner.supremeOwnerDashboardPage.посмотреть_всех_парт')}</Button>
                <Button
                  variant="outline"
                  size="sm"
                  ...
                  className="text-readable-xs"
                >
                  {t('owner.supremeOwnerDashboardPage.выбрать_весь_список')}</Button>
                {selectedPartners.size > 0 && (
                  <span className="text-readable-xs text-muted-high-contrast self-center">
                    {t('owner.supremeOwnerDashboardPage.выбрано')}{selectedPartners.size}
                  </span>
                )}
              </div>
              <div
            ...
            <div className="flex flex-col gap-4">
                ...
                {salesFunnel ? (
                  ...
                  <Card className="h-full xl:col-span-4">
                    <CardContent className="flex h-full items-center justify-center p-6 text-readable-sm text-muted-high-contrast">
                      {t('owner.supremeOwnerDashboardPage.нет_данных_по_воронк')}</CardContent>
                  </Card>
                )}
              </div>
              ...
              <div className="space-y-3">
                <div className="text-center">
                  <p className="text-xl font-normal text-slate-900 sm:text-2xl">{t('owner.supremeOwnerDashboardPage.активность_партнеров')}</p>
                  <p className="text-base text-slate-700">
                    {t('owner.supremeOwnerDashboardPage.топ_по_лидам_и_распр')}</p>
                </div>
                <div className="grid gap-4 lg:grid-cols-2">
                  <TopReferralsChart
              ...
              <div className="flex flex-col gap-3 rounded-xl border border-[color:var(--green-border)] bg-[color:var(--green-card)]/80 px-3 py-3 shadow-sm sm:flex-row sm:flex-wrap sm:items-center sm:px-4">
                <div className="min-w-0 flex-1">
                  <Input
                    ...
                    placeholder={t('owner.supremeOwnerDashboardPage.поиск_партн_ра')}
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Label htmlFor="owner-only-online" className="text-readable-sm font-normal text-high-contrast">
                    {t('owner.supremeOwnerDashboardPage.только_онлайн')}</Label>
                </div>
                <div className="flex items-center gap-2">
                  <Switch
                  ...
                  >
                    {t('owner.supremeOwnerDashboardPage.не_активные_7_дней')}</Label>
                </div>
              </div>
              <LeaderboardTable
          ...
          <DialogContent className="max-w-lg sm:max-w-lg">
            <DialogHeader>
              ...
              <DialogDescription>
                ...
                {selectedActivityPartners.length === 1 ? "партнёр" : "партнёров"} {t('owner.supremeOwnerDashboardPage.в_этой_группе')}</DialogDescription>
            </DialogHeader>
                ...
                >
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-readable-xs text-muted-high-contrast">
                      {t('owner.supremeOwnerDashboardPage.лиды')}{partner.leadsAdded.toLocaleString("ru-RU")} {t('owner.supremeOwnerDashboardPage.активность')}{" "}
                  ...
                  >
                    {t('owner.supremeOwnerDashboardPage.карточка')}</Button>
                </div>
              ))}
            </div>
          </DialogContent>
          ...
          <DialogContent className="max-h-[85vh] max-w-4xl overflow-hidden flex flex-col">
            <DialogHeader>
              <DialogTitle className="text-readable-xl font-medium text-high-contrast">
                {t('owner.supremeOwnerDashboardPage.все_партн_ры_сети')}</DialogTitle>
              <DialogDescription className="text-readable-base text-muted-high-contrast">
                {t('owner.supremeOwnerDashboardPage.полный_список_всех_п')}</DialogDescription>
            </DialogHeader>
                ...
                {globalData?.partners.map((partner) => (
                  ...
                  >
                    ...
                    <Avatar className="size-10 shrink-0">
                      <AvatarFallback className="text-readable-sm bg-gradient-to-br from-emerald-500 to-amber-500 text-white font-normal">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="text-readable-xs text-muted-high-contrast">
                        {t('owner.supremeOwnerDashboardPage.лиды')}{partner.leadsAdded.toLocaleString("ru-RU")} {t('owner.supremeOwnerDashboardPage.рефералы_l1')}{partner.level1Count} {t('owner.supremeOwnerDashboardPage.рефералы_l2')}{partner.level2Count}
                      </div>
                    </div>
                    <Button
                      variant="outline"
                      ...
                      className="text-readable-xs"
                    >
                      {t('owner.supremeOwnerDashboardPage.открыть')}</Button>
                  </div>
                ))}
              </div>
            </div>
            <div className="mt-4 border-t pt-4 flex justify-between items-center">
              <span className="text-readable-sm text-muted-high-contrast">
                {t('owner.supremeOwnerDashboardPage.всего_партн_ров')}{globalData?.partners.length || 0} {t('owner.supremeOwnerDashboardPage.выбрано')}{selectedPartners.size}
              </span>
              <div className="flex gap-2">
                <Button
                  ...
                  className="text-readable-xs"
                >
                  {t('owner.supremeOwnerDashboardPage.очистить_выбор')}</Button>
                <Button
                  size="sm"
                  onClick={() => {
                    setAllPartnersModalOpen(false)
                  ...
                  className="text-readable-xs"
                >
                  {t('owner.supremeOwnerDashboardPage.закрыть')}</Button>
              </div>
            </div>
          </DialogContent>
        </Dialog>
    ...
    <>
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between gap-2">
          <CardTitle className="text-readable-sm font-normal text-high-contrast">
            {"Расклад рефералов MLM"}
          </CardTitle>
          <span className="text-xs font-medium text-slate-600">{t('owner.supremeOwnerDashboardPage.3_линии_города')}</span>
        </div>
      </CardHeader>
      <CardContent className="space-y-3 pt-1">
        <p className="text-readable-sm text-high-contrast">
          {"Всего в структуре:"} <span className="font-normal">{partnersCount}</span>
        </p>
        <p className="text-readable-sm text-high-contrast">
          {"Текущий фокус:"} <span className="font-normal">{selectedLabel}</span>
        </p>
    ...
    >
      <div className="mb-3 flex items-center justify-between gap-2">
        <div className="min-w-0">
          <p className="truncate text-lg font-normal text-emerald-50">{branchRoot.label}</p>
          <p className="text-sm text-emerald-100/85">{t('owner.supremeOwnerDashboardPage.в_линии')}{branchPartnersCount} {t('owner.supremeOwnerDashboardPage.рефералов')}</p>
        </div>
        <Badge className="border-emerald-100/45 bg-emerald-900/70 px-2.5 py-1 text-sm font-normal text-emerald-50">
          {focusPartnerCount} {t('owner.supremeOwnerDashboardPage.рефералов')}</Badge>
      </div>
      ...
      <div className="rounded-2xl border border-emerald-800/70 bg-[radial-gradient(circle_at_center,rgba(6,78,59,0.34),rgba(2,44,34,0.82))] p-3">
        <div className="relative mx-auto w-full max-w-[360px] pt-4">
          ...
          >
            <span className="absolute right-3 top-2.5 text-lg font-normal text-slate-800/30">{"\u2666"}</span>
            <div className="flex items-center gap-3">
              <Avatar className="size-11 border-2 border-white shadow">
                ...
                <AvatarFallback className="bg-slate-700 text-base font-normal text-white">
                  {focusNode.label.slice(0, 2).toUpperCase()}
                </AvatarFallback>
              </Avatar>
              <div className="min-w-0">
                <p className="truncate text-lg font-normal text-slate-900">{focusNode.label}</p>
                <p className="text-base text-slate-600">{getRoleLabel(focusNode.role)}</p>
              </div>
            </div>
            <div className="mt-3 flex flex-wrap items-center gap-2">
              <span className="rounded-full border border-blue-200 bg-blue-50 px-2.5 py-0.5 text-sm font-normal text-blue-700">
                {t('owner.supremeOwnerDashboardPage.реф_l1')}{focusNode.childrenIds.length}
              </span>
              <span className="rounded-full border border-violet-200 bg-violet-50 px-2.5 py-0.5 text-sm font-normal text-violet-700">
                {t('owner.supremeOwnerDashboardPage.реф_l2')}{focusLevel2Count}
              </span>
            </div>
          </button>
        </div>
        ...
        <div className="mt-3 flex items-center justify-between gap-2">
          <p className="text-sm text-emerald-100/85">{t('owner.supremeOwnerDashboardPage.ниже_прямые_рефералы')}</p>
          <Button
            variant="outline"
            size="sm"
          ...
          {childNodes.length === 0 ? (
            ...
            childNodes.map((childNode) => {
              ...
              return (
                ...
                >
                  <div className="min-w-0">
                    <p className="truncate text-base font-normal text-slate-900">{childNode.label}</p>
                    <p className="text-sm text-slate-600">{t('owner.supremeOwnerDashboardPage.прямые')}{childNode.childrenIds.length}</p>
                  </div>
                  <span className="rounded-full border border-emerald-200 bg-emerald-50 px-2.5 py-0.5 text-sm font-normal text-emerald-700">
                    {childPartnerCount} {t('owner.supremeOwnerDashboardPage.рефералов')}</span>
                </button>
              )
            })
          )}
      ...
      >
        <DialogHeader className="shrink-0 border-b border-emerald-700/60 bg-emerald-900 px-6 py-4">
          <div className="flex items-start justify-between gap-4">
            <div className="min-w-0">
              <DialogTitle className="text-4xl font-normal text-emerald-50">
                {t('owner.supremeOwnerDashboardPage.карточный_стол_mlm')}</DialogTitle>
              <DialogDescription className="text-xl text-emerald-100/90">
                {t('owner.supremeOwnerDashboardPage.на_столе_только_карт')}</DialogDescription>
            </div>
            <Button
              variant="outline"
              size="sm"
          ...
          >
            <div className="mx-auto flex h-full w-full max-w-[1800px] flex-col gap-4 p-4 lg:flex-row">
              <section className="min-h-0 flex-1 rounded-3xl border border-emerald-800/70 bg-emerald-950/35 p-4 shadow-inner shadow-black/20">
                <div className="rounded-xl border border-emerald-700/70 bg-emerald-950/55 px-4 py-3 text-xl font-normal text-emerald-50">
                  {selectedNode
                ...
                {rootPartners.length === 0 ? (
                  <div className="mt-4 rounded-2xl border border-emerald-700/70 bg-emerald-950/55 px-4 py-6 text-lg text-emerald-100/90">
                    {t('owner.supremeOwnerDashboardPage.нет_данных_по_рефера')}</div>
                ) : (
                  <div
                    className={cn(
                      "mt-4 grid gap-4",
                  ...
                  <div className="rounded-2xl border border-slate-300 bg-white/85 p-3">
                    <div className="flex items-center gap-3">
                      <Avatar className="size-14 border-2 border-white shadow">
                        ...
                        <AvatarFallback className="bg-slate-500 text-lg font-normal text-white">
                          {(rootNode?.label ?? "Вы").slice(0, 2).toUpperCase()}
                        </AvatarFallback>
                      </Avatar>
                      <div className="min-w-0">
                        <p className="truncate text-2xl font-normal text-slate-900">
                          {rootNode?.label ?? "Вы"}
                        </p>
                        <p className="text-base text-slate-600">{t('owner.supremeOwnerDashboardPage.собственник')}</p>
                      </div>
                    </div>
                    <div className="mt-3 grid grid-cols-2 gap-2">
                      <div className="rounded-xl border border-slate-300 bg-slate-50 px-2.5 py-2">
                        <p className="text-sm text-slate-600">{t('owner.supremeOwnerDashboardPage.прямые')}</p>
                        <p className="text-2xl font-normal text-slate-900">
                          {rootPartners.length}
                        </p>
                      </div>
                      <div className="rounded-xl border border-slate-300 bg-slate-50 px-2.5 py-2">
                        <p className="text-sm text-slate-600">{t('owner.supremeOwnerDashboardPage.вся_ветка')}</p>
                        <p className="text-2xl font-normal text-slate-900">
                    ...
                    {onOpenOwnerCabinet && (
                      ...
                      >
                        {t('owner.supremeOwnerDashboardPage.кабинет_собственника')}</Button>
                    )}
                  </div>

                  <div>
                    <p className="mb-2 text-base font-normal uppercase tracking-wide text-slate-700">
                      {t('owner.supremeOwnerDashboardPage.рефералы_1_линии')}</p>
                    <div className="space-y-2">
                      {rootPartners.map((rootPartner) => (
                        <button
                          key={rootPartner.id}
                        ...
                        >
                          ...
                          >
                            {t('owner.supremeOwnerDashboardPage.реф_l1')}{rootPartner.childrenIds.length}
                          </Badge>
                        </button>
                      ))}
                    </div>
                  ...
                  {selectedNode && (
                    <div className="rounded-2xl border border-slate-300 bg-white/90 p-3">
                      ...
                      <div className="mt-2 grid grid-cols-2 gap-2">
                        <div className="rounded-xl border border-slate-300 bg-slate-50 px-2.5 py-2">
                          <p className="text-sm text-slate-600">{t('owner.supremeOwnerDashboardPage.рефералы_l1')}</p>
                          <p className="text-xl font-normal text-slate-900">
                            {selectedNode.childrenIds.length}
                          </p>
                        </div>
                        <div className="rounded-xl border border-slate-300 bg-slate-50 px-2.5 py-2">
                          <p className="text-sm text-slate-600">{t('owner.supremeOwnerDashboardPage.рефералы_l2')}</p>
                          <p className="text-xl font-normal text-slate-900">
                      ...
                      >
                        {t('owner.supremeOwnerDashboardPage.открыть_аналитику_ре')}</Button>
                      {cityId && selectedPersonId && (
                        ...
                        >
                          {t('owner.supremeOwnerDashboardPage.открыть_кабинет_рефе')}</Button>
                      )}
                    </div>
                  )}
                </div>
...
function PartnerAnalyticsPanel({
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full gap-4 p-8">
        <Button variant="ghost" onClick={onBack} className="self-start">
          ...
          {t('owner.supremeOwnerDashboardPage.назад')}</Button>
        <p className="text-muted-high-contrast">{t('owner.supremeOwnerDashboardPage.данные_реферала_не_н')}</p>
      </div>
    )
  }
  ...
  return (
    <div className="flex flex-col gap-6 p-6 max-w-2xl mx-auto w-full">
      ...
      >
        <ArrowLeft className="size-4 mr-2" />
        {t('owner.supremeOwnerDashboardPage.вернуться_к_карточно')}</Button>
      ...
      <div className="flex items-center gap-5">
        <div className="relative shrink-0">
          <Avatar className="size-24 border-4 border-white shadow-lg">
            ...
            <AvatarFallback className="text-2xl font-normal bg-gradient-to-br from-emerald-500 to-amber-500 text-white">
        ...
        <div className="min-w-0 flex flex-col gap-2">
          <h2 className="text-readable-2xl font-normal text-high-contrast truncate">
            {person.name}
          </h2>
          <div className="flex flex-wrap items-center gap-2">
            <Badge variant="outline" className="text-readable-xs">
        ...
        {kpiCards.map((kpi) => (
          <Card key={kpi.label} className={cn("border", kpi.colorClass)}>
            <CardContent className="flex flex-col items-center gap-1.5 p-4 text-center">
              {kpi.icon}
              <span className="text-2xl font-normal text-high-contrast">
