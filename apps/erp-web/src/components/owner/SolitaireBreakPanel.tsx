        ...
        <CardHeader className="pb-2">
          <CardTitle className="text-readable-sm text-high-contrast">
            <span className="inline-flex items-center gap-2">
              ...
              {t('owner.solitaireBreakPanel.мини_игры')}</span>
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-3 pt-1">
          <p className="text-readable-xs text-muted-high-contrast">{t('owner.solitaireBreakPanel.5_минут_перезагрузки')}</p>
          <Button type="button" className="w-full" onClick={() => setIsOpen(true)}>
            <Play className="mr-2 size-4" />
            {t('owner.solitaireBreakPanel.открыть_игровую_зону')}</Button>
        </CardContent>
      </Card>
          ...
          <div className="flex h-full w-full flex-col overflow-hidden border border-slate-700 bg-slate-950">
            <header className="flex items-start justify-between gap-3 border-b border-slate-700 px-4 py-3 sm:items-center sm:px-5">
              <div className="min-w-0">
                <h2 className="text-readable-lg font-normal text-white">{t('owner.solitaireBreakPanel.игровая_зона')}</h2>
                <p className="text-readable-xs text-slate-300">{t('owner.solitaireBreakPanel.выберите_игру')}</p>
              </div>
              <Button
                variant="outline"
                size="sm"
              ...
              >
                <X className="size-4" />
                {t('owner.solitaireBreakPanel.закрыть')}</Button>
            </header>
                ...
                <div className="space-y-3">
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className="size-12 overflow-hidden rounded-lg border border-slate-600 bg-slate-800">
                        <img src={sleepIconSrc} alt={t('owner.solitaireBreakPanel.менеджмент')} className="h-full w-full object-cover" />
                      </div>
                      <div>
                        <p className="text-readable-sm font-normal text-white">{t('owner.solitaireBreakPanel.менеджмент')}</p>
                        <p className="text-readable-xs text-slate-300">{t('owner.solitaireBreakPanel.разбудите_спящих_и_н')}</p>
                      </div>
                    </div>
                  </button>
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-readable-sm font-normal text-white">{t('owner.solitaireBreakPanel.тетрис')}</p>
                        <p className="text-readable-xs text-slate-300">{t('owner.solitaireBreakPanel.собирайте_линии_и_де')}</p>
                      </div>
                    </div>
                  </button>
                </div>
                  ...
                  <section className="space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
                      <div className="rounded-xl border border-slate-700 bg-slate-900/80 px-3 py-2">
                        <p className="text-readable-xs text-slate-300">{t('owner.solitaireBreakPanel.счет')}</p>
                        <p className="text-readable-lg font-normal text-white">{managementState.score}</p>
                      </div>
                      <div className="rounded-xl border border-slate-700 bg-slate-900/80 px-3 py-2">
                        <p className="text-readable-xs text-slate-300">{t('owner.solitaireBreakPanel.лучший')}</p>
                        <p className="text-readable-lg font-normal text-white">{managementBestScore}</p>
                      </div>
                      <div className="rounded-xl border border-slate-700 bg-slate-900/80 px-3 py-2">
                        <p className="text-readable-xs text-slate-300">{t('owner.solitaireBreakPanel.успешность')}</p>
                        <p className="text-readable-lg font-normal text-white">{managementSuccessRate}%</p>
                      </div>
                      <div className="rounded-xl border border-slate-700 bg-slate-900/80 px-3 py-2">
                        <p className="inline-flex items-center gap-1 text-readable-xs text-slate-300">
                          <Clock3 className="size-3.5" />
                          {t('owner.solitaireBreakPanel.время')}</p>
                        <p className="text-readable-lg font-normal text-white">
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      >
                        ...
                        {t('owner.solitaireBreakPanel.начать_заново')}</Button>
                      <p className="text-readable-xs text-slate-300">
                        {t('owner.solitaireBreakPanel.управление_wasd_или')}</p>
                    </div>

                    {managementReminder ? (
                      <div className="rounded-xl border border-amber-400/40 bg-amber-500/15 px-3 py-2 text-amber-100">
                        {t('owner.solitaireBreakPanel.5_минут_прошло_пора')}</div>
                    ) : null}
                  </section>
                ) : (
                  <section className="space-y-3">
                    <div className="grid grid-cols-2 gap-2 sm:grid-cols-5">
                      <div className="rounded-xl border border-slate-700 bg-slate-900/80 px-3 py-2">
                        <p className="text-readable-xs text-slate-300">{t('owner.solitaireBreakPanel.счет')}</p>
                        <p className="text-readable-sm font-normal text-white">{tetrisState.score}</p>
                      </div>
                      <div className="rounded-xl border border-slate-700 bg-slate-900/80 px-3 py-2">
                        <p className="text-readable-xs text-slate-300">{t('owner.solitaireBreakPanel.лучший')}</p>
                        <p className="text-readable-sm font-normal text-white">{tetrisState.bestScore}</p>
                      </div>
                      <div className="rounded-xl border border-slate-700 bg-slate-900/80 px-3 py-2">
                        <p className="text-readable-xs text-slate-300">{t('owner.solitaireBreakPanel.линии')}</p>
                        <p className="text-readable-sm font-normal text-white">{tetrisState.lines}</p>
                      </div>
                      <div className="rounded-xl border border-slate-700 bg-slate-900/80 px-3 py-2">
                        <p className="text-readable-xs text-slate-300">{t('owner.solitaireBreakPanel.уровень')}</p>
                        <p className="text-readable-sm font-normal text-white">{tetrisState.level}</p>
                      </div>
                      <div className="rounded-xl border border-slate-700 bg-slate-900/80 px-3 py-2">
                        <p className="inline-flex items-center gap-1 text-readable-xs text-slate-300">
                          <Clock3 className="size-3.5" />
                          {t('owner.solitaireBreakPanel.время')}</p>
                        <p className="text-readable-sm font-normal text-white">
                    ...
                    <div className="grid gap-3 xl:grid-cols-[300px_1fr]">
                      <div className="rounded-xl border border-slate-700 bg-slate-900 p-3">
                        <p className="text-readable-xs text-slate-300">{t('owner.solitaireBreakPanel.следующая_фигура')}</p>
                        ...
                        <p className="mt-3 text-readable-xs text-slate-300">
                          {t('owner.solitaireBreakPanel.управление_a_d_или_с')}</p>
                    ...
                    {tetrisState.isGameOver ? (
                      <div className="rounded-xl border border-rose-400/40 bg-rose-500/15 px-3 py-2 text-rose-100">
                        {t('owner.solitaireBreakPanel.игра_окончена_нажмит')}</div>
                    ) : null}
