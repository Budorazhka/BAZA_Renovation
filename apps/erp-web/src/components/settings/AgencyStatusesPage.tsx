    ...
    <DashboardShell>
      <div className="min-h-0 flex-1 overflow-y-auto p-6">
        <div className="mx-auto w-full max-w-6xl space-y-4">
          <div>
            <h1 className="text-xl font-normal text-[color:var(--theme-accent-heading)]">{t('settings.agencyStatusesPage.статусы_агентства_по')}</h1>
            <p className="mt-1 text-sm text-[color:var(--app-text-muted)]">
              {t('settings.agencyStatusesPage.ui_контроль_статусно')}</p>
          </div>
          ...
          <section className="rounded-lg border border-[var(--hub-card-border)] bg-[var(--hub-card-bg)] p-3">
            <div className="mb-3 flex items-center gap-2">
              ...
              <h2 className="text-sm font-normal text-[color:var(--theme-accent-heading)]">{t('settings.agencyStatusesPage.фильтры')}</h2>
            </div>
            <div className="grid grid-cols-1 gap-2 md:grid-cols-3">
              ...
              >
                <option value="all">{t('settings.agencyStatusesPage.регион_все')}</option>
                {regionOptions.map((r) => (
              ...
              <select
                ...
                className="rounded-md border border-[var(--workspace-row-border)] bg-[var(--workspace-row-bg)] px-2 py-2 text-sm text-[color:var(--workspace-text)]"
              >
                <option value="all">{t('settings.agencyStatusesPage.тип_статуса_все')}</option>
              ...
              <select
                ...
                className="rounded-md border border-[var(--workspace-row-border)] bg-[var(--workspace-row-bg)] px-2 py-2 text-sm text-[color:var(--workspace-text)]"
              >
                <option value="all">{t('settings.agencyStatusesPage.владение_все')}</option>
                <option value="mine">{t('settings.agencyStatusesPage.назначено_нам')}</option>
                <option value="free">{t('settings.agencyStatusesPage.свободные')}</option>
                <option value="other">{t('settings.agencyStatusesPage.у_других_агентств')}</option>
              </select>
            </div>
          </section>
          ...
          <div className="grid grid-cols-2 gap-2 md:grid-cols-4">
            <div className="rounded-lg border border-[var(--hub-card-border)] bg-[var(--hub-card-bg)] p-3">
              <p className="text-[10px] uppercase tracking-wide text-[color:var(--app-text-subtle)]">{t('settings.agencyStatusesPage.в_выборке')}</p>
              <p className="text-xl font-normal text-[color:var(--workspace-text)]">{stats.total}</p>
            </div>
            <div className="rounded-lg border border-[var(--hub-card-border)] bg-[var(--hub-card-bg)] p-3">
              <p className="text-[10px] uppercase tracking-wide text-[color:var(--app-text-subtle)]">{t('settings.agencyStatusesPage.наши_статусы')}</p>
              <p className="text-xl font-normal text-emerald-400">{stats.mine}</p>
            </div>
            <div className="rounded-lg border border-[var(--hub-card-border)] bg-[var(--hub-card-bg)] p-3">
              <p className="text-[10px] uppercase tracking-wide text-[color:var(--app-text-subtle)]">{t('settings.agencyStatusesPage.свободные')}</p>
              <p className="text-xl font-normal text-blue-400">{stats.free}</p>
            </div>
            <div className="rounded-lg border border-[var(--hub-card-border)] bg-[var(--hub-card-bg)] p-3">
              <p className="text-[10px] uppercase tracking-wide text-[color:var(--app-text-subtle)]">{t('settings.agencyStatusesPage.занятые_другими')}</p>
              <p className="text-xl font-normal text-amber-300">{stats.occupied}</p>
            </div>
          </div>
          ...
          <section className="rounded-lg border border-[var(--hub-card-border)] bg-[var(--hub-card-bg)] p-3">
            <div className="mb-3 flex items-center gap-2">
              ...
              <h2 className="text-sm font-normal text-[color:var(--theme-accent-heading)]">{t('settings.agencyStatusesPage.матрица_статусов')}</h2>
            </div>
              ...
              <table className="w-full min-w-[760px] border-collapse text-sm">
                <thead>
                  <tr className="border-b border-[color:var(--workspace-row-border)] text-left text-[11px] uppercase tracking-wide text-[color:var(--app-text-subtle)]">
                    <th className="px-2 py-2">{t('settings.agencyStatusesPage.регион')}</th>
                    <th className="px-2 py-2">{t('settings.agencyStatusesPage.тип_статуса')}</th>
                    <th className="px-2 py-2">{t('settings.agencyStatusesPage.владелец_статуса')}</th>
                    <th className="px-2 py-2">{t('settings.agencyStatusesPage.состояние')}</th>
                    <th className="px-2 py-2 text-right">{t('settings.agencyStatusesPage.действия')}</th>
                  </tr>
                </thead>
                <tbody>
                      ...
                      <tr key={row.id} className="border-b border-[color:var(--workspace-row-border)]">
                        ...
                        <td className="px-2 py-2">
                          {isMine ? (
                            <span className="inline-flex items-center gap-1 rounded-full border border-emerald-500/35 bg-emerald-500/10 px-2 py-0.5 text-xs text-emerald-300">
                              ...
                              {t('settings.agencyStatusesPage.назначен_нам')}</span>
                          ) : isFree ? (
                            <span className="inline-flex items-center gap-1 rounded-full border border-blue-500/35 bg-blue-500/10 px-2 py-0.5 text-xs text-blue-300">
                              ...
                              {t('settings.agencyStatusesPage.свободен')}</span>
                          ) : (
                            <span className="inline-flex items-center gap-1 rounded-full border border-amber-500/35 bg-amber-500/10 px-2 py-0.5 text-xs text-amber-300">
                              ...
                              {t('settings.agencyStatusesPage.конфликт_уникальност')}</span>
                          )}
                        </td>
                        <td className="px-2 py-2 text-right">
                          <div className="inline-flex items-center gap-2">
                            ...
                            >
                              {t('settings.agencyStatusesPage.назначить')}</button>
                            <button
                              type="button"
                            ...
                            >
                              {t('settings.agencyStatusesPage.снять')}</button>
                          </div>
                          {isConflict ? (
                            <p className="mt-1 text-right text-[11px] text-amber-300">
                              {t('settings.agencyStatusesPage.статус_уже_закреплен')}</p>
                          ) : null}
                        </td>
                      </tr>
                    )
            ...
            {filteredRows.length === 0 && (
              <p className="mt-2 text-sm text-[color:var(--app-text-muted)]">{t('settings.agencyStatusesPage.нет_строк_по_текущим')}</p>
            )}
          </section>
          ...
          <section className="rounded-lg border border-[var(--hub-card-border)] bg-[var(--hub-card-bg)] p-3">
            <div className="mb-2 flex items-center gap-2">
              ...
              <h2 className="text-sm font-normal text-[color:var(--theme-accent-heading)]">{t('settings.agencyStatusesPage.доступно_к_закреплен')}</h2>
            </div>
            <p className="mb-2 text-xs text-[color:var(--app-text-muted)]">{t('settings.agencyStatusesPage.свободные_ячейки_мат')}</p>
