    ...
    <div className="space-y-8 max-w-xl">
      ...
      <div className="rounded-xl border border-[color:var(--hub-card-border-hover)] bg-[var(--hub-action-hover)] p-5">
        <div className="flex items-start justify-between gap-4">
          <div>
            <div className="flex items-center gap-2">
              <span className="text-lg font-normal text-[color:var(--app-text)]">{t('settings.billingTab.бизнес')}</span>
              <span className="rounded-full border border-emerald-500/40 bg-emerald-500/15 px-2 py-0.5 text-[10px] font-normal text-emerald-400 uppercase tracking-wide">
                {t('settings.billingTab.активен')}</span>
            </div>
            <p className="mt-1 text-sm text-[color:var(--hub-desc)]">
              {t('settings.billingTab.следующее_списание')}<span className="text-[color:var(--app-text-muted)]">01.03.2026</span>
            </p>
          </div>
          <div className="text-right">
            <p className="text-2xl font-normal text-[color:var(--app-text)]">54 $</p>
        ...
        <div className="mt-4 flex gap-2">
          <button className="rounded-xl border border-[color:var(--hub-card-border-hover)] px-4 py-2 text-sm font-medium text-[color:var(--theme-accent-link-dim)] hover:border-[color:var(--hub-card-border-hover)] hover:text-[color:var(--app-text)] transition-colors">
            {t('settings.billingTab.изменить_тариф')}</button>
          <button className="rounded-xl border border-red-500/30 px-4 py-2 text-sm font-medium text-red-400/80 hover:border-red-400/50 hover:text-red-300 transition-colors">
            {t('settings.billingTab.отменить_подписку')}</button>
        </div>
      </div>
      ...
      <div className="space-y-4">
        <p className="text-xs font-medium uppercase tracking-wide text-[color:var(--hub-stat-label)]">
          {t('settings.billingTab.использование')}</p>
        <div className="rounded-xl border border-[color:var(--hub-tile-icon-border)] bg-[rgba(0,0,0,0.15)] p-5 space-y-5">
          <UsageBar label={t('settings.billingTab.объекты')}     used={26}    total={100}  icon={<TrendingUp className="size-3.5" />} />
          <UsageBar label={t('settings.billingTab.менеджеры')}   used={4}     total={10}   icon={<span className="text-xs">👤</span>} />
        </div>
      </div>
      ...
      <div className="space-y-3">
        <p className="text-xs font-medium uppercase tracking-wide text-[color:var(--hub-stat-label)]">
          {t('settings.billingTab.история_платежей')}</p>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[color:var(--hub-tile-icon-border)] bg-[rgba(0,0,0,0.2)]">
                <th className="px-4 py-2.5 text-left text-xs font-medium text-[color:var(--hub-stat-label)] uppercase tracking-wide">{t('settings.billingTab.дата')}</th>
                <th className="px-4 py-2.5 text-left text-xs font-medium text-[color:var(--hub-stat-label)] uppercase tracking-wide">{t('settings.billingTab.сумма')}</th>
                <th className="px-4 py-2.5 text-left text-xs font-medium text-[color:var(--hub-stat-label)] uppercase tracking-wide">{t('settings.billingTab.статус')}</th>
                <th className="px-4 py-2.5 text-right text-xs font-medium text-[color:var(--hub-stat-label)] uppercase tracking-wide">{t('settings.billingTab.сч_т')}</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-[color:var(--hub-card-border)]">
              {HISTORY.map((row) => (
                <tr key={row.id} className="hover:bg-[var(--hub-action-hover)] transition-colors">
                  ...
                  <td className="px-4 py-3 font-normal text-[color:var(--app-text)]">{row.amount}</td>
