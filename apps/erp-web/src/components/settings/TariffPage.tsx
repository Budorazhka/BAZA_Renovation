    ...
    <DashboardShell>
      <div style={{ padding: '24px 28px 48px' }}>
        <div style={{ marginBottom: 28 }}>
          <div style={{ fontSize: 20, fontWeight: 400, color: C.white, marginBottom: 4 }}>{t('settings.tariffPage.тарифные_планы')}</div>
          <div style={{ fontSize: 12, color: C.whiteLow }}>{t('settings.tariffPage.текущий_тариф')}<span style={{ color: C.gold, fontWeight: 400 }}>{t('settings.tariffPage.профи')}</span> {t('settings.tariffPage.следующее_списание')}</div>
        </div>
        ...
        <div className="mx-auto mb-6 max-w-6xl space-y-4">
          <section className="rounded-lg border border-[var(--hub-card-border)] bg-[var(--hub-card-bg)] p-3">
            <p className="mb-2 text-sm font-normal text-[color:var(--theme-accent-heading)]">{t('settings.tariffPage.биллинг_ui')}</p>
            <div className="flex flex-wrap gap-2">
              <button
                ...
                className={
                  billing === 'monthly'
                    ? 'rounded-md border border-[color:var(--gold)]/50 bg-[color:var(--gold)]/10 px-3 py-2 text-xs font-normal text-[color:var(--gold)]'
              ...
              >
                {t('settings.tariffPage.помесячно')}</button>
              <button
                type="button"
              ...
              >
                {t('settings.tariffPage.годовой_15')}</button>
            </div>
            <p className="mt-2 text-xs text-[color:var(--app-text-muted)]">
              {t('settings.tariffPage.отображение_цен_на_к')}</p>
          </section>
          ...
          <div className="grid grid-cols-1 gap-3 lg:grid-cols-2">
            <section className="rounded-lg border border-[var(--hub-card-border)] bg-[var(--hub-card-bg)] p-3">
              <div className="mb-2 flex items-center gap-2">
                ...
                <h2 className="text-sm font-normal text-[color:var(--theme-accent-heading)]">{t('settings.tariffPage.последние_счета')}</h2>
              </div>
                ...
                <table className="w-full border-collapse text-sm">
                  <thead>
                    <tr className="border-b border-[color:var(--workspace-row-border)] text-left text-[11px] uppercase text-[color:var(--app-text-subtle)]">
                      <th className="px-2 py-2">{t('settings.tariffPage.дата')}</th>
                      <th className="px-2 py-2">{t('settings.tariffPage.сумма')}</th>
                      <th className="px-2 py-2">{t('settings.tariffPage.план')}</th>
                      <th className="px-2 py-2">{t('settings.tariffPage.статус')}</th>
                    </tr>
                  </thead>
                  <tbody>
              ...
              <p className="mt-2 text-xs text-[color:var(--app-text-muted)]">{t('settings.tariffPage.период_оплаты')}{billing === 'monthly' ? 'ежемесячно' : 'ежегодно (предпросмотр)'}</p>
            </section>

            <section className="rounded-lg border border-[var(--hub-card-border)] bg-[var(--hub-card-bg)] p-3">
              <div className="mb-2 flex items-center gap-2">
                <AlertTriangle className="size-4 text-amber-400" />
                <h2 className="text-sm font-normal text-[color:var(--theme-accent-heading)]">{t('settings.tariffPage.лимиты')}</h2>
              </div>
                  ...
                  <li key={u.label} className="text-sm text-[color:var(--workspace-text)]">
                    <div className="flex justify-between gap-2">
                      ...
                      <span className={u.pct >= u.warn ? 'text-amber-300' : 'text-[color:var(--app-text-subtle)]'}>
                        {Math.round(u.pct * 100)}{t('settings.tariffPage.от_лимита')}</span>
                    </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 28 }}>
          {[
            ...
            <div key={s.label} style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 10, padding: '14px 16px' }}>
              ...
              {s.max === 0 ? (
                <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.25)' }}>{t('settings.tariffPage.недоступно')}</div>
              ) : (
                <>
                  <div style={{ fontSize: 16, fontWeight: 400, color: s.color, marginBottom: 6 }}>{s.used} / {s.max}</div>
            ...
            >
              {plan.current && (
                <div style={{
                  ...
                  {t('settings.tariffPage.текущий_план')}</div>
              )}

              {/* Icon + name */}
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
                <div style={{ color: plan.color }}>{plan.icon}</div>
                <div style={{ fontSize: 16, fontWeight: 400, color: C.white }}>{plan.name}</div>
              </div>

              {/* Price */}
              <div style={{ marginBottom: 20 }}>
                <span style={{ fontSize: 26, fontWeight: 400, color: plan.color }}>{plan.price}</span>
              ...
              <button
                style={{
                  ...
                  fontSize: 12, fontWeight: 400, cursor: plan.current ? 'default' : 'pointer',
        ...
        <div style={{ marginTop: 24, background: C.card, border: `1px solid ${C.border}`, borderRadius: 10, padding: '16px 20px', maxWidth: 500 }}>
          <div style={{ fontSize: 11, fontWeight: 400, letterSpacing: '0.1em', textTransform: 'uppercase', color: C.whiteLow, marginBottom: 10 }}>
            {t('settings.tariffPage.плат_жная_информация')}</div>
          {[
            ...
            <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '7px 0', borderBottom: i < 2 ? `1px solid rgba(255,255,255,0.05)` : 'none' }}>
              ...
              <span style={{ fontSize: 12, fontWeight: 400, color: C.whiteMid }}>{row.value}</span>
            </div>
          ))}
        </div>
      </div>
