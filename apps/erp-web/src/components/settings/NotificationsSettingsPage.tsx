    ...
    <DashboardShell>
      <div style={{ padding: '24px 28px 48px' }}>
        <div style={{ marginBottom: 24 }}>
          <div style={{ fontSize: 20, fontWeight: 400, color: C.white, marginBottom: 4 }}>{t('settings.notificationsSettingsPage.настройки_уведомлени')}</div>
          <div style={{ fontSize: 12, color: C.whiteLow }}>{t('settings.notificationsSettingsPage.управляйте_каналами')}</div>
        </div>
        ...
        <div className="mx-auto mb-5 max-w-6xl space-y-3">
          <div className="grid grid-cols-2 gap-2 md:grid-cols-4">
            <div className="rounded-lg border border-[var(--hub-card-border)] bg-[var(--hub-card-bg)] p-3">
              <p className="text-[10px] uppercase text-[color:var(--app-text-subtle)]">{t('settings.notificationsSettingsPage.событий_в_списке')}</p>
              <p className="text-xl font-normal text-[color:var(--workspace-text)]">{kpi.total}</p>
            </div>
            <div className="rounded-lg border border-[var(--hub-card-border)] bg-[var(--hub-card-bg)] p-3">
              <p className="text-[10px] uppercase text-[color:var(--app-text-subtle)]">{t('settings.notificationsSettingsPage.с_push')}</p>
              <p className="text-xl font-normal text-blue-300">{kpi.withPush}</p>
            </div>
            <div className="rounded-lg border border-[var(--hub-card-border)] bg-[var(--hub-card-bg)] p-3">
              <p className="text-[10px] uppercase text-[color:var(--app-text-subtle)]">{t('settings.notificationsSettingsPage.с_sms')}</p>
              <p className="text-xl font-normal text-amber-300">{kpi.withSms}</p>
            </div>
            <div className="rounded-lg border border-[var(--hub-card-border)] bg-[var(--hub-card-bg)] p-3">
              <p className="text-[10px] uppercase text-[color:var(--app-text-subtle)]">{t('settings.notificationsSettingsPage.хотя_бы_1_канал')}</p>
              <p className="text-xl font-normal text-emerald-300">{kpi.anyChannel}</p>
            </div>
          </div>
          <div className="rounded-lg border border-[var(--hub-card-border)] bg-[var(--hub-card-bg)] p-3">
            <div className="mb-2 flex items-center gap-2">
              <Filter className="size-4 text-[color:var(--gold)]" />
              <span className="text-sm font-normal text-[color:var(--theme-accent-heading)]">{t('settings.notificationsSettingsPage.фильтр_таблицы')}</span>
            </div>
            <div className="flex flex-col gap-2 sm:flex-row">
              ...
              >
                <option value="all">{t('settings.notificationsSettingsPage.все_группы')}</option>
                {GROUP_OPTIONS.map((g) => (
              ...
              <input
                ...
                placeholder={t('settings.notificationsSettingsPage.поиск_по_названию_со')}
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 640 }}>
            <thead>
              <tr style={{ borderBottom: `1px solid ${C.border}` }}>
                <th style={{ padding: '14px 18px', textAlign: 'left', fontSize: 11, color: C.whiteLow, fontWeight: 400, width: '40%' }}>
                  {t('settings.notificationsSettingsPage.событие')}</th>
                {CHANNELS.map(ch => (
                  <th key={ch.key} style={{ padding: '14px 10px', textAlign: 'center', minWidth: 90 }}>
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 11, fontWeight: 400, color: ch.color }}>
                        {ch.icon} {ch.label}
                      </div>
                      <div style={{ display: 'flex', gap: 4, fontSize: 9, color: C.whiteLow }}>
                        <button onClick={() => toggleAll(ch.key, true)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#4ade80', fontSize: 9, padding: '1px 4px' }}>{t('settings.notificationsSettingsPage.вс')}</button>
                        <span>/</span>
                        <button onClick={() => toggleAll(ch.key, false)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#ef4444', fontSize: 9, padding: '1px 4px' }}>{t('settings.notificationsSettingsPage.ничего')}</button>
                      </div>
                    </div>
                  </th>
                ))}
            ...
            <tbody>
              {Object.entries(groups).map(([group, rows]) => (
                <Fragment key={group}>
                  <tr>
                    <td colSpan={5} style={{
                      padding: '10px 18px 6px', fontSize: 10, fontWeight: 400,
        ...
        <div style={{ display: 'flex', gap: 10, marginTop: 20 }}>
          <button
            ...
            style={{
              padding: '10px 28px', borderRadius: 10, fontSize: 13, fontWeight: 400,
