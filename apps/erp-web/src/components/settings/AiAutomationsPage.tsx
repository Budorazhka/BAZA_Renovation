    ...
    <DashboardShell>
      <div className="min-h-0 flex-1 overflow-y-auto p-6">
        <div className="mx-auto w-full max-w-6xl space-y-4">
          <div>
            <h1 className="text-xl font-normal text-[color:var(--theme-accent-heading)]">{t('settings.aiAutomationsPage.ai_автоматизации')}</h1>
            <p className="mt-1 text-sm text-[color:var(--app-text-muted)]">
              {t('settings.aiAutomationsPage.сценарии_подсказок_и')}</p>
          </div>
          ...
          <section className="rounded-lg border border-[var(--hub-card-border)] bg-[var(--hub-card-bg)] p-3">
            <div className="mb-3 flex items-center gap-2">
              ...
              <h2 className="text-sm font-normal text-[color:var(--theme-accent-heading)]">{t('settings.aiAutomationsPage.фильтры')}</h2>
            </div>
            <div className="grid grid-cols-1 gap-2 md:grid-cols-2 lg:grid-cols-4">
              ...
              >
                <option value="all">{t('settings.aiAutomationsPage.модуль_все')}</option>
                <option value="leads">{t('settings.aiAutomationsPage.лиды')}</option>
                <option value="deals">{t('settings.aiAutomationsPage.сделки')}</option>
                <option value="docs">{t('settings.aiAutomationsPage.документы')}</option>
                <option value="objects">{t('settings.aiAutomationsPage.объекты')}</option>
              </select>
              <select
                value={priority}
                ...
                className="rounded-md border border-[var(--workspace-row-border)] bg-[var(--workspace-row-bg)] px-2 py-2 text-sm text-[color:var(--workspace-text)]"
              >
                <option value="all">{t('settings.aiAutomationsPage.приоритет_все')}</option>
                <option value="Высокий">{t('settings.aiAutomationsPage.высокий')}</option>
                <option value="Средний">{t('settings.aiAutomationsPage.средний')}</option>
                <option value="Низкий">{t('settings.aiAutomationsPage.низкий')}</option>
              </select>
              <label className="flex cursor-pointer items-center gap-2 rounded-md border border-[var(--workspace-row-border)] bg-[var(--workspace-row-bg)] px-2 py-2 text-sm text-[color:var(--workspace-text)]">
                ...
                {t('settings.aiAutomationsPage.только_включ_нные')}</label>
              <label className="flex cursor-pointer items-center gap-2 rounded-md border border-[var(--workspace-row-border)] bg-[var(--workspace-row-bg)] px-2 py-2 text-sm text-[color:var(--workspace-text)]">
                ...
                {t('settings.aiAutomationsPage.с_пдн_в_контексте')}</label>
            </div>
          </section>

          <section className="grid grid-cols-2 gap-2 md:grid-cols-5">
            <div className="rounded-lg border border-[var(--hub-card-border)] bg-[var(--hub-card-bg)] p-3">
              <p className="text-[10px] uppercase text-[color:var(--app-text-subtle)]">{t('settings.aiAutomationsPage.в_выборке')}</p>
              <p className="text-xl font-normal text-[color:var(--workspace-text)]">{kpi.total}</p>
            </div>
            <div className="rounded-lg border border-[var(--hub-card-border)] bg-[var(--hub-card-bg)] p-3">
              <p className="text-[10px] uppercase text-[color:var(--app-text-subtle)]">{t('settings.aiAutomationsPage.включено')}</p>
              <p className="text-xl font-normal text-emerald-300">{kpi.enabled}</p>
            </div>
            <div className="rounded-lg border border-[var(--hub-card-border)] bg-[var(--hub-card-bg)] p-3">
              <p className="text-[10px] uppercase text-[color:var(--app-text-subtle)]">{t('settings.aiAutomationsPage.запусков_7д')}</p>
              <p className="text-xl font-normal text-[color:var(--workspace-text)]">{money.format(kpi.runs)}</p>
            </div>
            <div className="rounded-lg border border-[var(--hub-card-border)] bg-[var(--hub-card-bg)] p-3">
              <p className="text-[10px] uppercase text-[color:var(--app-text-subtle)]">{t('settings.aiAutomationsPage.токенов_7д')}</p>
              <p className="text-xl font-normal text-amber-300">{money.format(kpi.tokens)}</p>
            </div>
            <div className="rounded-lg border border-[var(--hub-card-border)] bg-[var(--hub-card-bg)] p-3">
              <p className="text-[10px] uppercase text-[color:var(--app-text-subtle)]">{t('settings.aiAutomationsPage.высокий_приоритет')}</p>
              <p className="text-xl font-normal text-[color:var(--workspace-text)]">{kpi.high}</p>
            </div>
          </section>
          ...
          <section className="rounded-lg border border-[var(--hub-card-border)] bg-[var(--hub-card-bg)] p-3">
            <div className="mb-3 flex items-center gap-2">
              ...
              <h2 className="text-sm font-normal text-[color:var(--theme-accent-heading)]">{t('settings.aiAutomationsPage.сценарии')}</h2>
            </div>
              ...
              <table className="w-full min-w-[900px] border-collapse text-sm">
                <thead>
                  <tr className="border-b border-[color:var(--workspace-row-border)] text-left text-[11px] uppercase tracking-wide text-[color:var(--app-text-subtle)]">
                    <th className="px-2 py-2">{t('settings.aiAutomationsPage.сценарий')}</th>
                    <th className="px-2 py-2">{t('settings.aiAutomationsPage.модуль')}</th>
                    <th className="px-2 py-2">{t('settings.aiAutomationsPage.роли')}</th>
                    <th className="px-2 py-2">{t('settings.aiAutomationsPage.модель')}</th>
                    <th className="px-2 py-2">{t('settings.aiAutomationsPage.приоритет')}</th>
                    <th className="px-2 py-2">{t('settings.aiAutomationsPage.статус')}</th>
                    <th className="px-2 py-2">{t('settings.aiAutomationsPage.7д_запусков')}</th>
                    <th className="px-2 py-2">{t('settings.aiAutomationsPage.пдн')}</th>
                  </tr>
                </thead>
                <tbody>
                  {filtered.map((s) => (
          ...
          <section className="rounded-lg border border-[var(--hub-card-border)] bg-[var(--hub-card-bg)] p-3">
            <div className="mb-2 flex items-center gap-2">
              ...
              <h2 className="text-sm font-normal text-[color:var(--theme-accent-heading)]">{t('settings.aiAutomationsPage.контроль_нагрузки_и')}</h2>
            </div>
            <p className="mb-2 text-xs text-[color:var(--app-text-muted)]">{t('settings.aiAutomationsPage.высокочастотные_сцен')}</p>
            <ul className="space-y-2">
              {reviewPii.map((s) => (
                ...
                >
                  ...
                  {s.name} · {money.format(s.runs7d)} {t('settings.aiAutomationsPage.запусков_нед')}</li>
              ))}
            </ul>
          </section>
        </div>
