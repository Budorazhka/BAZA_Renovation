    ...
    <div className="flex flex-col items-center justify-center min-h-[400px] p-8">
      <div className="max-w-md w-full text-center">
        ...
        <h2 className="text-xl font-normal text-gray-900 mb-2">
          {t('common.moduleErrorBoundary.ошибка_загрузки_моду')}</h2>

        <p className="text-gray-600 mb-6">
          {moduleName && (
            <span className="font-medium">{t('common.moduleErrorBoundary.модуль')}{moduleName}</span>
          )}
          {moduleName && <br />}
          {t('common.moduleErrorBoundary.произошла_непредвиде')}</p>
          ...
          {import.meta.env.DEV && (
            <details className="text-xs text-gray-600">
              <summary className="cursor-pointer font-medium mb-2">
                {t('common.moduleErrorBoundary.детали_ошибки')}</summary>
        ...
        <div className="flex flex-col sm:flex-row gap-3 justify-center">
          <Button onClick={onReset} className="flex items-center gap-2">
            ...
            {t('common.moduleErrorBoundary.попробовать_снова')}</Button>
          <Button
            variant="outline"
            onClick={onGoHome}
          ...
          >
            ...
            {t('common.moduleErrorBoundary.на_главную')}</Button>
        </div>
      </div>
    </div>
  )
    ...
    return (
      <div className="flex items-center justify-center p-8 border border-red-200 bg-red-50 rounded-lg">
        <div className="text-center">
          ...
          <p className="text-sm text-red-800 font-medium">{moduleName} {t('common.moduleErrorBoundary.не_загрузился')}</p>
          <Button
            size="sm"
            variant="outline"
            onClick={onReset}
            className="mt-2"
          >
            {t('common.moduleErrorBoundary.обновить')}</Button>
        </div>
      </div>
    )
  }
    ...
    <div className="flex flex-col items-center justify-center p-8">
      <div className="text-center">
        ...
        <h3 className="text-lg font-normal text-gray-900 mb-2">
          {t('common.moduleErrorBoundary.ошибка_в_модуле')}{moduleName}
        </h3>
        <p className="text-gray-600 mb-4">{error.message}</p>
        <Button onClick={onReset}>{t('common.moduleErrorBoundary.перезагрузить_модуль')}</Button>
      </div>
    </div>
  )
}
