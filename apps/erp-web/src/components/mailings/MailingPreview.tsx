  ...
  return (
    ...
    >
      <div className="space-y-4">
        <h3 className="text-lg font-normal text-[color:var(--app-text)]">
          {title || 'Без названия'}
        </h3>
        <div
        ...
        {filteredLinks.length > 0 && (
          <div className="space-y-2">
            <p className="text-sm font-medium text-[color:var(--app-text)]">{t('mailings.mailingPreview.ссылки')}</p>
        ...
        {fileUrl && fileName && (
          <div className="flex items-center gap-2 rounded-md border border-[var(--green-border)] bg-[var(--green-card)] px-3 py-2">
            ...
            >
              {t('mailings.mailingPreview.скачать')}</a>
          </div>
        )}
      </div>
    </div>
