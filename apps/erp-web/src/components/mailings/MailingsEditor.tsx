  ...
  return (
    ...
    <Card className={CARD_SURFACE}>
      <CardHeader className="space-y-1.5 pb-4">
        <CardTitle className="text-xl text-[color:var(--app-text)]">{t('mailings.mailingsEditor.редактор_рассылок')}</CardTitle>
        <CardDescription className="text-base text-[color:var(--app-text-muted)]">
          {t('mailings.mailingsEditor.новости_оповещения_и')}</CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        <div className="space-y-2">
          <Label htmlFor="mailing-title" className={LABEL}>
            {t('mailings.mailingsEditor.название_оповещения')}</Label>
          <Input
            id="mailing-title"
            value={title}
            onChange={(e) => setTitle(e.target.value)}
            placeholder={t('mailings.mailingsEditor.введите_название')}
            className={`text-base ${FIELD}`}
          />
        </div>

        <div className="space-y-2">
          <Label className={LABEL}>{t('mailings.mailingsEditor.текст_оповещения')}</Label>
          <RichTextEditor
            value={body}
            onChange={setBody}
            placeholder={t('mailings.mailingsEditor.введите_текст_можно')}
          />
        </div>
        ...
        <div className="space-y-3">
          <Label className={LABEL}>{t('mailings.mailingsEditor.когда_отправить')}</Label>
          <div className="flex flex-col gap-2">
            <label className="flex cursor-pointer items-center gap-2">
              <input
                ...
                className="size-4"
              />
              <span className="text-base text-[color:var(--app-text)]">{t('mailings.mailingsEditor.отправить_сейчас')}</span>
            </label>
            <label className="flex cursor-pointer items-center gap-2">
              <input
                type="radio"
                ...
                className="size-4"
              />
              <span className="text-base text-[color:var(--app-text)]">{t('mailings.mailingsEditor.запланировать')}</span>
            </label>
            {!sendNow && (
              <div className="ml-6">
                <Input
        ...
        <div className="flex flex-wrap gap-3">
          ...
          >
            {t('mailings.mailingsEditor.предпросмотр')}</Button>
          <Button
            type="button"
    ...
    <Dialog open={previewOpen} onOpenChange={setPreviewOpen}>
      ...
      >
        <DialogHeader>
          <DialogTitle className="text-[color:var(--app-text)]">{t('mailings.mailingsEditor.предпросмотр_рассылк')}</DialogTitle>
        </DialogHeader>
        <div className="space-y-4 pt-2">
          <div>
            <p className="mb-2 text-sm font-medium text-[color:var(--app-text-muted)]">{t('mailings.mailingsEditor.как_в_crm')}</p>
            <MailingPreview
              title={title}
              body={body || ''}
          ...
          <div>
            <p className="mb-2 text-sm font-medium text-[color:var(--app-text-muted)]">{t('mailings.mailingsEditor.как_в_личном_кабинет')}</p>
            <MailingPreview
              title={title}
              body={body || ''}
              links={links.filter((l) => l.url.trim())}
    ...
    {mailings.length > 0 && (
      <Card className={`mt-8 ${CARD_SURFACE}`}>
        <CardHeader className="space-y-1.5 pb-4">
          <CardTitle className="text-xl text-[color:var(--app-text)]">{t('mailings.mailingsEditor.история_рассылок')}</CardTitle>
          <CardDescription className="text-base text-[color:var(--app-text-muted)]">
            {t('mailings.mailingsEditor.отправленные_и_запла')}</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-3">
            {mailings.map((m) => {
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {isScheduled && m.scheduledAt && (
                      <p className="mt-1 text-sm text-[color:var(--theme-accent-heading)]">
                        {t('mailings.mailingsEditor.запланировано_на')}{formatDate(m.scheduledAt)}
                      </p>
                    )}
                  </div>
                  {isScheduled && (
                    <Button
                      ...
                      className="border-[var(--green-border)] bg-transparent text-[color:var(--app-text)] hover:bg-[var(--dropdown-hover)]"
                    >
                      {t('mailings.mailingsEditor.отменить')}</Button>
                  )}
                </div>
              )
            })}
