      ...
      ? createPortal(
          <>
            ...
            <div
              role="dialog"
              aria-modal
              aria-label={t('clients.conversionFunnelCard.настройка_конверсии')}
              style={{
                position: 'fixed',
                zIndex: 10001,
                top: '50%',
            ...
            >
            <h3 style={{ margin: '0 0 8px', fontSize: 17, fontWeight: 400, color: 'var(--app-text)' }}>
              {t('clients.conversionFunnelCard.конверсия_по_этапам')}</h3>
            <p style={{ margin: '0 0 18px', fontSize: 14, color: 'var(--app-text-muted)', lineHeight: 1.55 }}>
              {t('clients.conversionFunnelCard.считаем_долю_лидов_к')}<strong style={{ color: 'var(--app-text)' }}>{t('clients.conversionFunnelCard.конечного')}</strong> {t('clients.conversionFunnelCard.этапа_среди_тех_кто')}<strong style={{ color: 'var(--app-text)' }}>{t('clients.conversionFunnelCard.стартовом')}</strong> {t('clients.conversionFunnelCard.этапе_или_прош_л_его')}<strong style={{ color: 'var(--app-text)' }}>{monthLabelRu()}</strong>.
            </p>

            <div style={{ fontSize: 12, fontWeight: 400, color: PRIMARY, letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 10 }}>
              {t('clients.conversionFunnelCard.быстрые_пресеты')}</div>
                ...
                return (
                  ...
                  >
                    ...
                    <span style={{ display: 'block', fontSize: 13, fontWeight: 500, color: 'var(--app-text-muted)', marginTop: 4 }}>
                      {p.value}% · {p.delta} {t('clients.conversionFunnelCard.к_прошлому_месяцу')}</span>
                  </button>
                )
              })}
            </div>

            <div style={{ fontSize: 12, fontWeight: 400, color: PRIMARY, letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 12 }}>
              {t('clients.conversionFunnelCard.своя_связка')}</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <label style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                <span style={{ fontSize: 14, fontWeight: 400, color: 'var(--app-text-muted)' }}>{t('clients.conversionFunnelCard.с_этапа')}</span>
                <select
                  value={fromId}
                  style={selectStyle}
              ...
              <label style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                <span style={{ fontSize: 14, fontWeight: 400, color: 'var(--app-text-muted)' }}>{t('clients.conversionFunnelCard.до_этапа')}</span>
                <select
            ...
            <p style={{ margin: '18px 0 0', fontSize: 14, color: 'var(--app-text-muted)', lineHeight: 1.55 }}>
              <strong style={{ color: 'var(--app-text)' }}>{t('clients.conversionFunnelCard.как_читать')}</strong> {t('clients.conversionFunnelCard.из_лидов_прошедших')}{fromStage?.name ?? '—'}», {value === '—' ? 'метрика для этой пары пока не заведена в моке' : `${value}% дошли до «${stageById(toId)?.name ?? '—'}»`} {t('clients.conversionFunnelCard.за_выбранный_месяц_м')}</p>
            ...
            >
              {t('clients.conversionFunnelCard.готово')}</button>
            </div>
          </>,
          document.body,
        )
        ...
        >
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
              <span style={{ fontSize: 9, fontWeight: 400, color: PRIMARY, letterSpacing: '0.12em', textTransform: 'uppercase' }}>
                {t('clients.conversionFunnelCard.конверсия_базы')}</span>
              <BarChart3 size={18} color="var(--gold)" />
            </div>
            <h4 style={{ margin: 0, fontSize: 18, fontWeight: 400, color: 'var(--app-text)', lineHeight: 1.2 }}>
              {value === '—' ? '—' : `${value}%`}
            </h4>
            <div style={{ marginTop: 6, display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 8 }}>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 11, fontWeight: 400, color: 'var(--app-text-muted)', lineHeight: 1.35 }}>
                  {pairLabel}
                </div>
                {delta ? (
                  <div style={{ marginTop: 2, fontSize: 11, fontWeight: 400, color: deltaColor }}>
                    {delta} {t('clients.conversionFunnelCard.к_прошлому_месяцу')}</div>
                ) : null}
                <div style={{ marginTop: 6, fontSize: 10, color: 'var(--app-text-subtle)', lineHeight: 1.3 }}>
                  {t('clients.conversionFunnelCard.нажмите_чтобы_сменит')}</div>
              </div>
              <ChevronDown
                size={20}
                color="var(--gold)"
