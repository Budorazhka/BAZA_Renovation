  ...
  if (!city) {
    return (
      <div className="flex min-h-[50vh] items-center justify-center bg-[var(--app-bg)] text-[color:var(--app-text-muted)]">
        <p>{t('city.cityMailingsPage.город_не_найден')}</p>
      </div>
    )
  }
    ...
    <div className="space-y-6 text-[color:var(--app-text)]">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          >
            <Link to={`/dashboard/city/${city.id}`} aria-label={t('city.cityMailingsPage.назад_к_городу')}>
              <ArrowLeft className="size-5" />
            </Link>
          </Button>
          <div>
            <h1 className="text-2xl font-normal tracking-tight text-[color:var(--app-text)]">{t('city.cityMailingsPage.редактор_рассылок')}</h1>
      ...
      <p className="text-xs text-[color:var(--app-text-muted)]">
        {t('city.cityMailingsPage.сводка_и_история_ниж')}</p>

      <div className="grid grid-cols-2 gap-2 md:grid-cols-5">
        <div className="rounded-lg border border-[var(--hub-card-border)] bg-[var(--hub-card-bg)] p-3">
          <p className="text-[10px] uppercase text-[color:var(--app-text-subtle)]">{t('city.cityMailingsPage.всего')}</p>
          <p className="text-xl font-normal text-[color:var(--gold)]">{mailingKpi.total}</p>
        </div>
        <div className="rounded-lg border border-[var(--hub-card-border)] bg-[var(--hub-card-bg)] p-3">
          <p className="text-[10px] uppercase text-[color:var(--app-text-subtle)]">{t('city.cityMailingsPage.отправлено')}</p>
          <p className="text-xl font-normal text-emerald-300">{mailingKpi.sent}</p>
        </div>
        <div className="rounded-lg border border-[var(--hub-card-border)] bg-[var(--hub-card-bg)] p-3">
          <p className="text-[10px] uppercase text-[color:var(--app-text-subtle)]">{t('city.cityMailingsPage.в_плане')}</p>
          <p className="text-xl font-normal text-sky-300">{mailingKpi.scheduled}</p>
        </div>
        <div className="rounded-lg border border-[var(--hub-card-border)] bg-[var(--hub-card-bg)] p-3">
          <p className="text-[10px] uppercase text-[color:var(--app-text-subtle)]">{t('city.cityMailingsPage.просрочено')}</p>
          <p className="text-xl font-normal text-orange-300">{mailingKpi.overdue}</p>
        </div>
        <div className="rounded-lg border border-[var(--hub-card-border)] bg-[var(--hub-card-bg)] p-3 md:col-span-1 col-span-2">
          <p className="text-[10px] uppercase text-[color:var(--app-text-subtle)]">{t('city.cityMailingsPage.crm_лк')}</p>
          <p className="text-xl font-normal text-violet-300">{mailingKpi.dualChannel}</p>
        </div>
      </div>

      {overdueMailings.length > 0 && (
        <div className="rounded-xl border border-orange-500/35 bg-orange-500/[0.07] p-4">
          <div className="mb-2 flex items-center gap-2 text-[11px] font-normal uppercase tracking-wide text-orange-200">
            <AlertTriangle className="size-3.5 shrink-0" />
            {t('city.cityMailingsPage.запланировано_но_вре')}</div>
          <ul className="m-0 list-none space-y-1.5 p-0">
            {overdueMailings.map((m: Mailing) => (
              ...
              >
                <span className="font-normal text-[color:var(--app-text)]">{m.title}</span>
                {m.scheduledAt && (
                  <span className="ml-2 text-[10px]">
                    {t('city.cityMailingsPage.план')}{new Date(m.scheduledAt).toLocaleString('ru-RU', { dateStyle: 'short', timeStyle: 'short' })}
                  </span>
                )}
              </li>
            ))}
