    ...
    <div className="space-y-8">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            >
              ...
              {t('city.cityDetailView.направить_рассылку')}</Button>
            <Button onClick={() => setAddDialogOpen(true)} size="lg" className="text-base">
              <Plus className="mr-2 size-5" />
              {t('city.cityDetailView.добавить_партнера')}</Button>
          </div>
        </div>
              ...
              <CardContent className="p-0 px-6">
                <div className="flex items-center gap-4">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-2xl font-normal tracking-tight">{stat.value}</p>
      ...
      <Card>
        <CardHeader className="space-y-1.5 pb-4">
          <CardTitle className="text-xl">{t('city.cityDetailView.партнеры_города')}</CardTitle>
          <CardDescription className="text-base">{t('city.cityDetailView.иерархия_city_master')}</CardDescription>
        </CardHeader>
        <CardContent className="overflow-x-auto">
          {hierarchyRows.length === 0 ? (
            <p className="py-10 text-center text-base text-muted-foreground">{t('city.cityDetailView.партнеры_еще_не_доба')}</p>
          ) : (
            <table className="w-full min-w-[1040px] text-base">
              <thead>
                <tr className="border-b border-slate-200 text-left text-sm uppercase tracking-wide text-muted-foreground">
                  <th className="py-3 pr-5">{t('city.cityDetailView.партнер')}</th>
                  <th className="py-3 pr-5">{t('city.cityDetailView.роль')}</th>
                  <th className="py-3 pr-5">{t('city.cityDetailView.объекты')}</th>
                  <th className="py-3 pr-5">{t('city.cityDetailView.время_в_системе')}</th>
                  <th className="py-3 pr-5">{t('city.cityDetailView.активность')}</th>
                  <th className="py-3 pr-5">{t('city.cityDetailView.онлайн_7д')}</th>
                  <th className="py-3 text-right">{t('city.cityDetailView.действия')}</th>
                </tr>
              </thead>
              <tbody>
                    ...
                    >
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              {isMaster && (
                                <span className="rounded-full bg-amber-50 px-2.5 py-1 text-sm font-medium text-amber-700">
                                  {t('city.cityDetailView.мастер_города')}</span>
                              )}
                            </div>
                      ...
                      <td className="py-4 pr-5">
                        <div className="flex flex-wrap gap-2">
                          {partner.roles.length > 0 ? (
                            ...
                            <span className="text-sm text-muted-foreground">{t('city.cityDetailView.роли_не_назначены')}</span>
                          )}
                        </div>
                      </td>
                      <td className="py-4 pr-5">
                      ...
                      <td className="py-4 pr-5">
                        ...
                        <p className="mt-1 text-sm text-muted-foreground">
                          {t('city.cityDetailView.лиды')}{row.leadsAdded} {t('city.cityDetailView.сделки')}{row.deals}
                        </p>
                      </td>
                      <td className="py-4 pr-5">
                        <div className="flex items-center gap-2">
