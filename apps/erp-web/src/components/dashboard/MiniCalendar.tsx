...
export function MiniCalendar({
  ...
  onTodayReactivate,
    ...
    const { t } = useI18n();
  const { calendarEvents } = useCrmSync()
  ...
  const dayNumClass = isWorkspace
    ...
    : 'flex size-6 shrink-0 items-center justify-center rounded-full text-[11px] font-normal leading-none'
  const headerMonthClass = isWorkspace
    ? 'text-sm font-normal tracking-tight text-[color:var(--workspace-text)] sm:text-base'
    : 'text-sm font-normal tracking-tight text-[color:var(--workspace-text)]'
  const dowClass = isWorkspace
    ? 'text-center text-[11px] font-normal uppercase tracking-wider text-[color:var(--workspace-text-dim)]'
    : 'text-center text-[9px] font-normal uppercase tracking-wider text-[color:var(--workspace-text-dim)]'
      ...
      <div className={cn('flex shrink-0 items-center justify-between gap-1.5', isWorkspace ? 'mb-0.5' : 'mb-3')}>
        ...
        <div className="flex shrink-0 items-center gap-0.5">
          <button
            ...
            aria-label={t('dashboard.miniCalendar.предыдущий_месяц')}
          >
          ...
          <button
            ...
            className={cn(
              ...
              isWorkspace ? 'px-1.5 py-px text-[9px] font-normal' : 'px-2 py-0.5 text-[10px] font-normal',
            )}
          >
            {t('dashboard.miniCalendar.сегодня')}</button>
          <button
            type="button"
            onClick={nextMonth}
            className={cn(
            ...
            aria-label={t('dashboard.miniCalendar.следующий_месяц')}
          >
                ...
                className={cn(
                  ...
                  isToday
                    ? cn(
                        ...
                        isWorkspace ? 'font-normal' : 'font-normal',
                      )
        ...
        >
          <div
            className={cn(
              ...
              isWorkspace ? 'mb-0.5 text-[10px] font-normal' : 'mb-1.5 text-[10px] font-normal',
            )}
          >
          ...
          {selectedEvents.length === 0 ? (
            <p className="text-[10px] text-[color:var(--workspace-text-dim)]">{t('dashboard.miniCalendar.нет_событий')}</p>
          ) : (
            <ul
              className={cn(
                'space-y-0.5',
            ...
            >
              ...
              {isWorkspace && selectedEvents.length > 2 ? (
                <li className="text-[9px] text-[color:var(--workspace-text-dim)]">
                  {t('dashboard.miniCalendar.ещ')}{selectedEvents.length - 2}
                </li>
              ) : null}
            </ul>
          )}
      ...
      {!hideFooterLink ? (
        <div className={cn(isWorkspace ? 'mt-1.5 text-center' : 'mt-2 text-right')}>
          <Link
            ...
            className="text-[11px] font-normal uppercase tracking-wider text-[color:var(--theme-accent-link-dim)] underline-offset-2 hover:text-[color:var(--theme-accent-link)] hover:underline"
          >
            {t('dashboard.miniCalendar.полный_календарь')}</Link>
        </div>
      ) : null}
    </div>
  )
