    ...
    <DashboardShell>
      <div className="min-h-0 flex-1 overflow-y-auto p-6">
        <div className="mx-auto w-full max-w-6xl space-y-4">
          <div>
            <h1 className="text-xl font-normal text-[color:var(--theme-accent-heading)]">{t('team.branchesPage.филиалы')}</h1>
            <p className="mt-1 text-sm text-[color:var(--app-text-muted)]">
              {t('team.branchesPage.реестр_филиалов_стру')}</p>
          </div>
          ...
          <div className="grid min-h-[440px] grid-cols-1 gap-3 lg:grid-cols-[1.1fr_1fr]">
            <section className="rounded-lg border border-[var(--hub-card-border)] bg-[var(--hub-card-bg)] p-3">
              <div className="mb-2 flex items-center gap-2">
                ...
                <h2 className="text-sm font-normal text-[color:var(--theme-accent-heading)]">{t('team.branchesPage.список_филиалов')}</h2>
              </div>
              <div className="space-y-2">
                {BRANCHES.map((branch) => (
                  ...
                  >
                    <p className="text-sm font-normal text-[color:var(--workspace-text)]">{branch.name}</p>
            ...
            <section className="rounded-lg border border-[var(--hub-card-border)] bg-[var(--hub-card-bg)] p-3">
              <div className="mb-2 flex items-center gap-2">
                ...
                <h2 className="text-sm font-normal text-[color:var(--theme-accent-heading)]">{t('team.branchesPage.карточка_филиала')}</h2>
              </div>
              {active ? (
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div className="rounded-md border border-[color:var(--workspace-row-border)] bg-[var(--workspace-row-bg)] p-2">
                      <p className="text-[11px] text-[color:var(--app-text-subtle)]">{t('team.branchesPage.руководитель')}</p>
                    ...
                    <div className="rounded-md border border-[color:var(--workspace-row-border)] bg-[var(--workspace-row-bg)] p-2">
                      <p className="flex items-center gap-1 text-[11px] text-[color:var(--app-text-subtle)]">
                        ...
                        {t('team.branchesPage.сотрудники')}</p>
                      <p className="mt-1 text-sm font-normal text-[color:var(--workspace-text)]">{active.employees}</p>
                    </div>
                  </div>

                  <div className="rounded-md border border-[color:var(--workspace-row-border)] bg-[var(--workspace-row-bg)] p-2">
                    <p className="text-[11px] text-[color:var(--app-text-subtle)]">{t('team.branchesPage.команды_филиала')}</p>
