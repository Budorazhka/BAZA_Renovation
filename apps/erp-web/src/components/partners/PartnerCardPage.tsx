    ...
    return (
      <DashboardShell>
        <div style={{ ...S.page, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ color: 'var(--app-text-subtle)', fontSize: 14 }}>{t('partners.partnerCardPage.партн_р_не_найден')}</div>
        </div>
      </DashboardShell>
    )
  }
  ...
  return (
    <DashboardShell>
      <div style={S.page}>
        ...
        >
          <ArrowLeft size={14} />
          {t('partners.partnerCardPage.все_партн_ры')}</button>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 20, marginBottom: 28 }}>
          <div style={{
            ...
            fontSize: 22, fontWeight: 400, color: S.gold,
          }}>
            {partner.name[0]}
          </div>
          <div style={{ flex: 1 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 4 }}>
              <span style={{ fontSize: 20, fontWeight: 400, color: S.gold }}>{partner.name}</span>
              <span style={{
          ...
          <div style={{ display: 'flex', gap: 12, flexShrink: 0 }}>
            {[
              ...
              <div key={k.label} style={{ ...S.card, textAlign: 'center', padding: '12px 16px' }}>
                ...
                <div style={{ fontSize: 15, fontWeight: 400, color: k.color }}>{k.value}</div>
        ...
        {tab === 'profile' && (
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            <div style={S.card}>
              <div style={{ fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.1em', color: S.dim, marginBottom: 12 }}>{t('partners.partnerCardPage.контактные_данные')}</div>
              {[
            ...
            <div style={S.card}>
              <div style={{ fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.1em', color: S.dim, marginBottom: 12 }}>{t('partners.partnerCardPage.финансовая_сводка')}</div>
              {[
                ...
                <div key={row.label} style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 0', borderBottom: `1px solid ${S.rowLine}` }}>
                  <span style={{ fontSize: 12, color: S.dim }}>{row.label}</span>
                  <span style={{ fontSize: 14, fontWeight: 400, color: row.color }}>{row.value}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {tab === 'leads' && (
          <div style={S.card}>
            {partner.leads.length === 0 && (
              <div style={{ textAlign: 'center', padding: '32px 0', color: S.dim, fontSize: 13 }}>{t('partners.partnerCardPage.нет_переданных_лидов')}</div>
            )}
            {partner.leads.map((lead, i) => (
              <div key={lead.id} style={{
                ...
                <div>
                  <div style={{ fontSize: 13, fontWeight: 400, color: S.text }}>{lead.clientName}</div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                  {lead.commission != null && lead.commission > 0 && (
                    <span style={{ fontSize: 13, fontWeight: 400, color: '#4ade80' }}>{FMT.format(lead.commission)}</span>
                  )}
                  <span style={{
        ...
        {tab === 'payouts' && (
          <div style={S.card}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              <span style={{ fontSize: 12, color: S.dim }}>{t('partners.partnerCardPage.история_выплат')}</span>
              <button
                type="button"
                style={{
                  padding: '6px 14px', borderRadius: 'var(--section-cta-radius)', fontSize: 11, fontWeight: 400,
                  background: 'rgba(74,222,128,0.1)', border: '1px solid rgba(74,222,128,0.3)',
                  color: '#4ade80', cursor: 'pointer', letterSpacing: '0.05em',
                }}
              >
                {t('partners.partnerCardPage.начислить')}</button>
            </div>
            {partner.payouts.length === 0 && (
              <div style={{ textAlign: 'center', padding: '32px 0', color: S.dim, fontSize: 13 }}>{t('partners.partnerCardPage.нет_выплат')}</div>
            )}
            {partner.payouts.map((payout, i) => (
              <div key={payout.id} style={{
                ...
                <div>
                  <div style={{ fontSize: 13, fontWeight: 400, color: S.text }}>{FMT.format(payout.amount)}</div>
                  <div style={{ fontSize: 11, color: S.dim }}>
                    {new Date(payout.date).toLocaleDateString('ru')} {t('partners.partnerCardPage.сделка')}{payout.dealId}
                  </div>
            ...
            {partner.payouts.length > 0 && (
              <div style={{ marginTop: 16, paddingTop: 12, borderTop: `1px solid ${S.rowLine}`, display: 'flex', justifyContent: 'space-between' }}>
                <span style={{ fontSize: 12, color: S.dim }}>{t('partners.partnerCardPage.итого_выплачено')}</span>
                <span style={{ fontSize: 15, fontWeight: 400, color: '#4ade80' }}>
