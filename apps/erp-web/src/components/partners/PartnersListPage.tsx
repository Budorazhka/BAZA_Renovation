...
const S = {
  ...
  title: { fontSize: 22, fontWeight: 400, color: 'var(--theme-accent-heading)', marginBottom: 4 } as React.CSSProperties,
...
export function PartnersListPage() {
  ...
  return (
    <DashboardShell>
      <div style={S.page}>
        <div style={S.title}>{t('partners.partnersListPage.партн_ры')}</div>
        <div style={S.sub}>{t('partners.partnersListPage.рефералы_посредники')}</div>

        <p className="mb-4 text-[11px] text-[color:var(--app-text-subtle)]">
          {t('partners.partnersListPage.в_базе')}<span className="font-normal text-[color:var(--workspace-text)]">{totalInDb}</span> {t('partners.partnersListPage.записей_kpi_ниже_счи')}</p>

        <div className="mb-6 grid grid-cols-2 gap-2 md:grid-cols-4">
          <div className="rounded-xl border border-[var(--hub-card-border)] bg-[var(--hub-card-bg)] p-4">
            <div className="text-[10px] font-normal uppercase tracking-wide text-[color:var(--app-text-subtle)]">{t('partners.partnersListPage.в_выдаче')}</div>
            <div className="mt-1 text-xl font-normal text-[color:var(--gold)]">{kpi.count}</div>
            <div className="mt-0.5 text-[10px] text-[color:var(--app-text-subtle)]">{t('partners.partnersListPage.активных')}{kpi.active}</div>
          </div>
          <div className="rounded-xl border border-[var(--hub-card-border)] bg-[var(--hub-card-bg)] p-4">
            <div className="text-[10px] font-normal uppercase tracking-wide text-[color:var(--app-text-subtle)]">{t('partners.partnersListPage.лидов_передано')}</div>
            <div className="mt-1 text-xl font-normal text-blue-300">{kpi.leads}</div>
          </div>
          <div className="rounded-xl border border-[var(--hub-card-border)] bg-[var(--hub-card-bg)] p-4">
            <div className="text-[10px] font-normal uppercase tracking-wide text-[color:var(--app-text-subtle)]">{t('partners.partnersListPage.комиссия_всего')}</div>
            <div className="mt-1 text-xl font-normal text-emerald-300">{FMT.format(kpi.commission)}</div>
          </div>
          <div className="rounded-xl border border-[var(--hub-card-border)] bg-[var(--hub-card-bg)] p-4">
            <div className="text-[10px] font-normal uppercase tracking-wide text-[color:var(--app-text-subtle)]">{t('partners.partnersListPage.баланс_к_выплате')}</div>
            <div className="mt-1 text-xl font-normal text-amber-200">{FMT.format(kpi.balance)}</div>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', gap: 12, marginBottom: 20, alignItems: 'center', flexWrap: 'wrap' }}>
          <div style={{
            ...
            <input
              ...
              placeholder={t('partners.partnersListPage.имя_телефон_email')}
              value={search}
          ...
          >
            <option value="name">{t('partners.partnersListPage.по_имени')}</option>
            <option value="leads-desc">{t('partners.partnersListPage.по_лидам')}</option>
            <option value="balance-desc">{t('partners.partnersListPage.по_балансу')}</option>
            <option value="newest">{t('partners.partnersListPage.сначала_новые')}</option>
          </select>
          <button type="button" className="alphabase-section-primary">
            <Plus size={12} strokeWidth={2.5} />
            {t('partners.partnersListPage.добавить_партн_ра')}</button>
        </div>
        ...
        {attention.length > 0 && (
          ...
          >
            <div className="mb-2 flex items-center gap-2 text-[11px] font-normal uppercase tracking-wide text-amber-200">
              <AlertTriangle className="size-3.5 shrink-0" />
              {t('partners.partnersListPage.требуют_внимания')}</div>
              ...
              {attention.map((p) => {
                ...
                return (
                  <li key={p.id}>
                    ...
                    >
                      <span className="font-normal text-[color:var(--app-text)]">{p.name}</span>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {filtered.length === 0 && (
            <div style={{ textAlign: 'center', padding: '48px 0', color: 'var(--app-text-subtle)', fontSize: 13 }}>
              {t('partners.partnersListPage.нет_партн_ров_по_зад')}</div>
          )}
          {filtered.map(partner => (
            ...
            >
              ...
              <div style={{
                ...
                fontSize: 16, fontWeight: 400, color: TYPE_COLORS[partner.type],
              }}>
                {partner.name[0]}
              </div>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 2 }}>
                  <span style={{ fontSize: 14, fontWeight: 400, color: 'var(--app-text)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {partner.name}
                  </span>
                  <span style={{
              ...
              <div style={{ display: 'flex', gap: 24, flexShrink: 0, alignItems: 'center' }}>
                <div style={{ textAlign: 'center' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 4, justifyContent: 'center' }}>
                    ...
                    <span style={{ fontSize: 15, fontWeight: 400, color: '#60a5fa' }}>{partner.leadsCount}</span>
                  </div>
                  <div style={{ fontSize: 9, color: 'var(--app-text-subtle)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>{t('partners.partnersListPage.лидов')}</div>
                </div>
                <div style={{ textAlign: 'center' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 4, justifyContent: 'center' }}>
                    <Wallet size={11} color="var(--app-text-subtle)" />
                    <span style={{ fontSize: 15, fontWeight: 400, color: partner.balance > 0 ? '#4ade80' : 'var(--app-text-subtle)' }}>
                      {FMT.format(partner.balance)}
                    </span>
                  </div>
                  <div style={{ fontSize: 9, color: 'var(--app-text-subtle)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>{t('partners.partnersListPage.баланс')}</div>
                </div>
        ...
        {kpi.balance > 0 && (
          <div style={{
            ...
            <span style={{ fontSize: 13, color: 'var(--app-text-muted)' }}>
              {t('partners.partnersListPage.баланс_к_выплате_в_в')}<strong style={{ color: '#4ade80' }}>{FMT.format(kpi.balance)}</strong>
            </span>
          </div>
        )}
      </div>
