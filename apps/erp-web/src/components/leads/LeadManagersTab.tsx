    ...
    <div className="space-y-5 max-w-2xl">
      ...
      {!canManageTeam && (
        <div className="flex items-start gap-3 rounded-xl border border-[color:var(--hub-card-border)] bg-[rgba(0,0,0,0.15)] px-4 py-3">
          ...
          <p className="text-sm text-[color:var(--hub-stat-label)]">
            {t('leads.leadManagersTab.добавление_и_редакти')}</p>
        </div>
      )}
      ...
      <div className="rounded-xl border border-[color:var(--hub-card-border)] bg-[rgba(0,0,0,0.15)] overflow-hidden">
        ...
        <div className="flex items-center justify-between gap-3 px-5 py-4 border-b border-[color:var(--hub-tile-icon-border)]">
          <h3 className="font-normal text-[color:var(--app-text)]">{t('leads.leadManagersTab.менеджеры_по_лидам')}</h3>
          {canManageTeam && (
            <button
              type="button"
              onClick={openAdd}
            ...
            >
              ...
              {t('leads.leadManagersTab.добавить_менеджера')}</button>
          )}
        </div>
          ...
          {leadManagers.map((m) => (
            <li key={m.id} className="flex flex-wrap items-center justify-between gap-3 px-5 py-3.5 transition-colors hover:bg-[var(--hub-action-hover)]">
              <div className="flex items-center gap-3">
                <div className="flex size-9 shrink-0 items-center justify-center rounded-full bg-[var(--hub-tile-icon-bg)] text-sm font-normal text-[color:var(--app-text)]">
                  {m.name.charAt(0)}
                </div>
                <div>
                ...
                {canManageTeam && (
                  <>
                    ...
                    >
                      ...
                      {t('leads.leadManagersTab.редактировать')}</button>
                    <button
                      type="button"
                    ...
                    >
                      {t('leads.leadManagersTab.удалить')}</button>
                  </>
                )}
              </div>
            </li>
      ...
      <Dialog open={open} onOpenChange={(v) => !v && closeDialog()}>
        <DialogContent showCloseButton={false} className="max-w-md rounded-2xl border border-[color:var(--hub-card-border)] bg-[linear-gradient(180deg,rgba(9,36,28,0.99),rgba(6,20,16,0.98))] p-0 shadow-2xl">
          <div className="flex items-center justify-between border-b border-[color:var(--hub-tile-icon-border)] px-6 py-4">
            <h2 className="text-base font-normal text-[color:var(--app-text)]">
          ...
          <div className="space-y-4 px-6 py-5">
            <div>
              <label className={LABEL}>{t('leads.leadManagersTab.логин_email')}</label>
              <input value={login} onChange={(e) => setLogin(e.target.value)} placeholder="manager@example.com" className={FIELD} />
            </div>
            <div>
              <label className={LABEL}>{t('leads.leadManagersTab.имя')}</label>
              <input value={name} onChange={(e) => setName(e.target.value)} placeholder={t('leads.leadManagersTab.иван_менеджеров')} className={FIELD} />
            </div>
            <div>
              <label className={LABEL}>{t('leads.leadManagersTab.очереди_лидов')}</label>
              ...
              <p className="mt-2 text-[11px] text-[color:var(--theme-accent-icon-dim)]">
                {t('leads.leadManagersTab.выберите_очереди_к_к')}</p>
            </div>
          </div>
          ...
          <div className="flex items-center justify-end gap-2 border-t border-[color:var(--hub-tile-icon-border)] px-6 py-4">
            ...
            >
              {t('leads.leadManagersTab.отмена')}</button>
            <button
              type="button"
              onClick={handleSave}
              disabled={!login.trim() || !name.trim() || sourceTypes.length === 0}
              className="rounded-full bg-emerald-600 px-5 py-2 text-sm font-normal text-white hover:bg-emerald-500 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
            >
