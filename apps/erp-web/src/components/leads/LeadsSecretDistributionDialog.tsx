    ...
    >
      ...
      >
        <DialogHeader className="sr-only">
          <DialogTitle>{t('leads.leadsSecretDistributionDialog.режим_руководителя_у')}</DialogTitle>
          <DialogDescription>
            {t('leads.leadsSecretDistributionDialog.маршрутизация_лидов')}</DialogDescription>
        </DialogHeader>
          ...
          <div className="secret-table-shell" style={{ zIndex: STACK_ORDER.hud }}>
            <header className="secret-table-hud">
              <div className="secret-title">
                ...
                <div>
                  <p className="secret-kicker">{t('leads.leadsSecretDistributionDialog.режим_руководителя')}</p>
                  <h2 className="secret-heading">{t('leads.leadsSecretDistributionDialog.управление_потоком_з')}</h2>
                </div>
              </div>
              <div className="secret-hud-controls">
                <div className="secret-search-box">
                  ...
                  <input
                    ...
                    placeholder={t('leads.leadsSecretDistributionDialog.поиск_по_id_или_теле')}
                  />
                  {trimmedSearch && (
                    <div className="secret-search-results">
                      {searchResults.length === 0 ? (
                        <p className="secret-search-empty">{t('leads.leadsSecretDistributionDialog.по_запросу_ничего_не')}</p>
                      ) : (
                ...
                {!isMobile && (
                  <div className="secret-view-toggle">
                    ...
                    >
                      ...
                      {t('leads.leadsSecretDistributionDialog.стол')}</button>
                    <button
                      type="button"
                    ...
                    >
                      ...
                      {t('leads.leadsSecretDistributionDialog.список')}</button>
                  </div>
                )}
              </div>
                ...
                <HoverHint
                  ...
                  label={
                    <>
                      ...
                      {t('leads.leadsSecretDistributionDialog.в_очереди')}{unassignedNewLeads.length}
                ...
                <HoverHint
                  ...
                  label={
                    <>
                      ...
                      {t('leads.leadsSecretDistributionDialog.горячие')}{hotLeadsCount}
                    </>
                  }
                  hint={
                    <span className="block space-y-2">
                      <strong>{t('leads.leadsSecretDistributionDialog.горячие_лиды_score_2')}</strong> {t('leads.leadsSecretDistributionDialog.считаются_по_скору')}<ul className="list-disc pl-4 space-y-1 mt-1">
                        <li>{t('leads.leadsSecretDistributionDialog.комиссия_от_5_000_по')}</li>
                        <li>{t('leads.leadsSecretDistributionDialog.2ч_без_разбора_лид_д')}</li>
                        <li>{t('leads.leadsSecretDistributionDialog.просрочка_по_задаче')}</li>
                      </ul>
                      {t('leads.leadsSecretDistributionDialog.чем_выше_скор_тем_пр')}</span>
                  }
                />
                <HoverHint
                  className="secret-hud-metric"
                  label={
                    <>
                      ...
                      {t('leads.leadsSecretDistributionDialog.фильтр')}{activeDeckFilterLabel}
                    </>
                  }
                ...
                <HoverHint
                  ...
                  label={
                    <>
                      ...
                      {t('leads.leadsSecretDistributionDialog.назначено')}{sessionTouchedLeadIds.length}
                    </>
                  }
              ...
              >
                ...
                {t('leads.leadsSecretDistributionDialog.закрыть')}</Button>
            </header>
                  ...
                  <div className="secret-mobile-deck">
                          ...
                          <span className="secret-card-face secret-card-front">
                            ...
                            {isLeadStale(activeDeckLead, nowTs) && (
                              <span className="secret-card-stale-badge">
                                ...
                                {t('leads.leadsSecretDistributionDialog.2ч')}</span>
                            )}
                            <span className="secret-card-front-name">{activeDeckLead.name ?? activeDeckLead.id}</span>
                            <div className="secret-card-front-facts">
                              <span>
                                <strong>{t('leads.leadsSecretDistributionDialog.комиссия')}</strong>
                                {formatLeadBudget(activeDeckLead)}
                              </span>
                              <span>
                                <strong>{t('leads.leadsSecretDistributionDialog.тип')}</strong>
                                {SOURCE_LABELS[activeDeckLead.source]}
                              </span>
                              <span>
                                <strong>{t('leads.leadsSecretDistributionDialog.источник')}</strong>
                                {getLeadChannelLabel(activeDeckLead)}
                              </span>
                            </div>
                            <span className="secret-card-front-stage">{t('leads.leadsSecretDistributionDialog.статус')}{selectedLeadStage}</span>
                          </span>
                        </button>
                      ) : (
                        <div className="secret-top-card is-empty">
                          <span>{t('leads.leadsSecretDistributionDialog.колода_пуста')}</span>
                        </div>
                      )}
                    </div>
                    <p className="secret-deck-caption">{t('leads.leadsSecretDistributionDialog.статус_новый_лид')}</p>
                    ...
                    <div className="secret-mobile-actions">
                      ...
                      >
                        ...
                        {t('leads.leadsSecretDistributionDialog.отменить')}</Button>
                      <Button
                        type="button"
                      ...
                      >
                        ...
                        {t('leads.leadsSecretDistributionDialog.сброс')}</Button>
                    </div>
                  </div>
                    ...
                    return (
                      <article key={manager.id} className="secret-mobile-manager">
                        <div className="secret-mobile-manager-head">
                          ...
                          <div>
                            ...
                            <p
                              ...
                              title={t('leads.leadsSecretDistributionDialog.проблемные_просрочка')}
                            >
                              {t('leads.leadsSecretDistributionDialog.проблемных')}{managerProblemLoad[manager.id] ?? 0} {t('leads.leadsSecretDistributionDialog.всего')}{managerLoad[manager.id] ?? 0}
                            </p>
                          </div>
                        </div>
                        ...
                        <div className="secret-manager-actions">
                          ...
                          >
                            {t('leads.leadsSecretDistributionDialog.назначить_в_руку')}</Button>
                        </div>
                      </article>
                    )
                  })}
              ...
              <section className="secret-main-layout">
                {viewMode === 'table' ? (
                    ...
                    {leadManagers.map((manager, index) => {
                      ...
                      return (
                        ...
                        >
                            ...
                            <div className="secret-seat-head-meta">
                              ...
                              <div className="secret-seat-load">
                                <span
                                  ...
                                  title={t('leads.leadsSecretDistributionDialog.лиды_в_рабочих_этапа')}
                                >
                                  {t('leads.leadsSecretDistributionDialog.в_работе')}{managerInWork}
                                </span>
                          ...
                          >
                            <span className="secret-seat-hand-label">{t('leads.leadsSecretDistributionDialog.рука')}</span>
                          ...
                          <div
                            ...
                            title={t('leads.leadsSecretDistributionDialog.проблемные_просрочка')}
                          >
                            {t('leads.leadsSecretDistributionDialog.проблемных')}{managerProblem} {t('leads.leadsSecretDistributionDialog.всего')}{managerTotal}
                          </div>
                        </article>
                      )
                    })}
                  ...
                  <div className="secret-felt-lead-summary" style={{ zIndex: STACK_ORDER.cards }}>
                    <div className="secret-felt-summary-inner">
                      <p className="secret-felt-summary-title">{t('leads.leadsSecretDistributionDialog.сводка')}</p>
                      {selectedLead ? (
                        <div className="secret-felt-summary-body">
                          ...
                          <p className="secret-felt-summary-sub">
                            {t('leads.leadsSecretDistributionDialog.комиссия')}{formatLeadBudget(selectedLead)} · {getLeadChannelLabel(selectedLead)}
                          </p>
                          <p className="secret-felt-summary-sub">ID: {selectedLead.id}</p>
                        </div>
                      ) : (
                        <p className="secret-felt-summary-empty">{t('leads.leadsSecretDistributionDialog.выберите_заявку_из_п')}</p>
                      )}
                  ...
                  <div className="secret-deck-pit" style={{ zIndex: STACK_ORDER.cards }}>
                        ...
                        >
                          ...
                          <span className="secret-card-face secret-card-front">
                            ...
                            <span className="secret-card-front-stage">
                              {t('leads.leadsSecretDistributionDialog.статус')}{STAGE_NAME_BY_ID[activeDeckLead.stageId] ?? activeDeckLead.stageId}
                            </span>
                          </span>
                        </button>
                      ) : (
                        <div className="secret-top-card is-empty">
                          <span>{t('leads.leadsSecretDistributionDialog.новых_лидов_нет')}</span>
                        </div>
                      )}
                    </div>
                    <p className="secret-deck-caption">
                      {t('leads.leadsSecretDistributionDialog.центральный_пул_толь')}</p>
                    <div className="secret-deck-remaining-badge">
                      {t('leads.leadsSecretDistributionDialog.в_очереди')}{unassignedNewLeads.length}
                      {deckFilter !== 'all' ? ` / ${allUnassignedNewLeads.length}` : ''}
                    </div>
                    ...
                    <div className="secret-deck-actions">
                      ...
                      >
                        <RotateCcw className="size-4" />
                        {t('leads.leadsSecretDistributionDialog.отменить_ход')}</Button>
                    </div>
                  </div>
                  ...
                  <div className="secret-list-view">
                    <div className="secret-list-head">
                      <p className="secret-list-title">{t('leads.leadsSecretDistributionDialog.пул_лидов')}</p>
                    ...
                    <div className="secret-list-table-wrap">
                      <table className="secret-list-table">
                        <thead>
                          <tr>
                            <th>ID</th>
                            <th>{t('leads.leadsSecretDistributionDialog.клиент')}</th>
                            <th>{t('leads.leadsSecretDistributionDialog.телефон')}</th>
                            <th>{t('leads.leadsSecretDistributionDialog.тип')}</th>
                            <th>{t('leads.leadsSecretDistributionDialog.комиссия')}</th>
                            <th>{t('leads.leadsSecretDistributionDialog.метки')}</th>
                            <th />
                          </tr>
                        </thead>
                        <tbody>
                          {unassignedNewLeads.length === 0 ? (
                            <tr>
                              <td colSpan={7} className="secret-list-empty">
                                {t('leads.leadsSecretDistributionDialog.по_текущему_фильтру')}</td>
                            </tr>
                          ) : (
                            unassignedNewLeads.map((lead) => {
                              const leadPhone = getLeadPhoneValue(lead)
                                ...
                                <tr key={`list-${lead.id}`} className={cn(activeDeckLead?.id === lead.id && 'is-active')}>
                                  ...
                                  <td>
                                    <Button
                                      ...
                                      onClick={() => focusLeadFromQueue(lead.id)}
                                    >
                                      {t('leads.leadsSecretDistributionDialog.в_фокус')}</Button>
                                  </td>
                                </tr>
                              )
                            })
                ...
                <aside className="secret-side-panel" style={{ zIndex: STACK_ORDER.modal }}>
                  <div className="secret-side-card">
                    <p className="secret-side-title">{t('leads.leadsSecretDistributionDialog.правила')}</p>
                    <div className="secret-rule-line">
                      <Flame className="size-4 shrink-0 mt-0.5" />
                      <div>
                        <p className="font-normal mb-1">{t('leads.leadsSecretDistributionDialog.горячие_score')}{HOT_MIN_SCORE})</p>
                        <p className="text-sm opacity-90">
                          {t('leads.leadsSecretDistributionDialog.комиссия_от')}{HOT_BUDGET_USD.toLocaleString('ru-RU')}{t('leads.leadsSecretDistributionDialog.2ч_без_разбора_прос')}</p>
                      </div>
                    </div>
                    <p className="secret-rule-line">
                      <ShieldAlert className="size-4" />
                      {t('leads.leadsSecretDistributionDialog.в_очереди_только_лид')}</p>
                    <p className="secret-rule-line">
                      <ShieldAlert className="size-4" />
                      {t('leads.leadsSecretDistributionDialog.назначение_выполняет')}</p>
                    <p className="secret-rule-line">
                      <RefreshCw className="size-4" />
                      {t('leads.leadsSecretDistributionDialog.сброс_откатывает_тол')}</p>
                  </div>

                  {showAllCards && (
                    <div className="secret-side-card secret-all-list-card">
                      <p className="secret-side-title">{t('leads.leadsSecretDistributionDialog.пул_лидов')}</p>
                      <div className="secret-all-list">
                        {unassignedNewLeads.length === 0 ? (
                          <p className="secret-side-empty">{t('leads.leadsSecretDistributionDialog.в_очереди_нет_новых')}</p>
                        ) : (
                          unassignedNewLeads.map((lead) => (
                            <button
                              key={lead.id}
          ...
          {drawerManager && (
            <>
              <button
                ...
                aria-label={t('leads.leadsSecretDistributionDialog.закрыть_досье_менедж')}
              />
              <aside className="secret-manager-drawer is-open" style={{ zIndex: STACK_ORDER.modal + 2 }}>
                <div className="secret-manager-drawer-head">
                  <div>
                    <p className="secret-manager-drawer-kicker">{t('leads.leadsSecretDistributionDialog.досье_менеджера')}</p>
                ...
                <section className="secret-manager-drawer-section">
                  <p className="secret-manager-drawer-section-title">{t('leads.leadsSecretDistributionDialog.воронка_статусов')}</p>
                  <div className="secret-manager-stage-chips">
                    {drawerStageRows.length === 0 ? (
                      <p className="secret-manager-drawer-empty">{t('leads.leadsSecretDistributionDialog.нет_активных_лидов')}</p>
                    ) : (
                ...
                <section className="secret-manager-drawer-section">
                  <p className="secret-manager-drawer-section-title">{t('leads.leadsSecretDistributionDialog.финансовая_сводка')}</p>
                  <div className="secret-manager-finance-wrap">
                    <table className="secret-manager-finance-table">
                      <thead>
                        <tr>
                          <th>{t('leads.leadsSecretDistributionDialog.срез')}</th>
                          <th>{t('leads.leadsSecretDistributionDialog.лиды')}</th>
                          <th>{t('leads.leadsSecretDistributionDialog.потенциал')}</th>
                        </tr>
                      </thead>
                      <tbody>
                  ...
                  <p className="secret-manager-finance-note">
                    {t('leads.leadsSecretDistributionDialog.успех')}{SUCCESS_STAGE_NAMES.join(', ')}.
                  </p>
                </section>

                <section className="secret-manager-drawer-section secret-manager-drawer-cards-section">
                  <p className="secret-manager-drawer-section-title">{t('leads.leadsSecretDistributionDialog.активные_карты')}</p>
                  <div className="secret-manager-lead-cards">
                    {drawerLeads.length === 0 ? (
                      <p className="secret-manager-drawer-empty">{t('leads.leadsSecretDistributionDialog.у_менеджера_нет_лидо')}</p>
                    ) : (
                ...
                <section className="secret-manager-drawer-section">
                  ...
                  >
                    {t('leads.leadsSecretDistributionDialog.забрать_просроченные')}{drawerOverdueLeadIds.length})
                  </Button>
                </section>
              </aside>
            </>
