...
export type ThemeType = 'felt' | 'standard' | 'light' | 'light-green'
export type ThemePreference = 'auto' | 'standard' | 'light' | 'light-green'
...
interface ThemeContextValue {
  theme: ThemeType
  isFeltStyle: boolean
  /** true для любой светлой темы (light и light-green) */
...
function readPreferenceFromStorage(): ThemePreference {
  try {
    ...
    if (raw === 'auto' || raw === 'standard' || raw === 'light' || raw === 'light-green') return raw as ThemePreference
  } catch {
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  const themeValue = useMemo<ThemeContextValue>(() => {
    ...
    if (preference === 'light') {
      theme = 'light'
    } else if (preference === 'light-green') {
      theme = 'light-green'
    ...
    const isLightTheme = theme === 'light' || theme === 'light-green'
    return { theme, isFeltStyle, isLightTheme, preference, setPreference, uiFontScale, setUiFontScale }
