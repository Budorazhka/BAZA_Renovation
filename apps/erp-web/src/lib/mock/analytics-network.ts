...
/** Веса колонок для распределения мок-чисел (сумма ≈ 1 на воронку). */
const FUNNEL_COLUMN_WEIGHTS: Record<FunnelId, Record<string, number>> = {
    sales: { rejection: 0.32, in_progress: 0.53, success: 0.15 },
    network: { rejection: 0.34, in_progress: 0.56, active: 0.1 },
    owner: { rejection: 0.3, preparation: 0.42, in_progress: 0.28 },
    broker: { rejection: 0.34, in_progress: 0.5, active: 0.16 },
};

function csvDefToFunnelTemplate(def: CsvFunnelDefinition): FunnelTemplate {
    const weights = FUNNEL_COLUMN_WEIGHTS[def.id];
    return {
        id: def.id,
        name: def.name,
        ...
        columns: def.columns.map((column) => ({
            id: column.id,
            name: column.name,
            weight: weights[column.id] ?? 0.33,
            stages: column.stages,
        })),
    };
}

/** Этапы и подписи — из CSV «ВОРОНКИ - …» (`src/data/funnel-stages-from-csv.ts`). */
const funnelTemplates: FunnelTemplate[] = [
    csvDefToFunnelTemplate(FUNNEL_SALES_CSV),
    csvDefToFunnelTemplate(FUNNEL_NETWORK_CSV),
    csvDefToFunnelTemplate(FUNNEL_OWNER_CSV),
    csvDefToFunnelTemplate(FUNNEL_BROKER_CSV),
];
