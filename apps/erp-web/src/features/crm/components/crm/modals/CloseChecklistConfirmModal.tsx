...
interface CloseChecklistConfirmModalProps {
  isOpen: boolean;
  onCancel: () => void;
  onConfirm: () => void;
}

export const CloseChecklistConfirmModal: React.FC<CloseChecklistConfirmModalProps> = ({
  isOpen,
  onCancel,
  onConfirm,
}) => {
  ...
  return (
    <div className="modal-fade-in fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[70] pt-4 md:pt-4 pb-0 md:pb-4 px-0 md:px-4" onClick={onCancel}>
      <div 
      ...
      >
        <div className="flex justify-between items-center px-12.5 py-4 border-b border-gray-200">
          <span className="text-dream-primary text-lg font-normal">{t('closeChecklistConfirm.title')}</span>
        </div>
