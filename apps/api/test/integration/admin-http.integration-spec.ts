...
describe('Admin HTTP routes — integration (полный AppModule, реальные Fastify hooks)', () => {
  ...
  async function seedRealAdmin(isSuperAdmin: boolean): Promise<{ login: string; cookie: string; adminAccountId: string; identityId: Types.ObjectId }> {
    ...
    return { login, cookie: `baza_session=${session.sessionToken}`, adminAccountId: account._id.toString(), identityId };
  }

  describe('аутентификация (401 vs 403, non-disclosure)', () => {
    it('запрос без cookie — 401 AUTH_NO_SESSION; невалидная cookie по-прежнему остаётся non-disclosed 403', async () => {
