 ...
 * `resource.action` формату, что и остальная матрица.
 *
 * ДОДЕЛАНО 01.09.2026. Ровно та проблема, которую этот докстринг и
 * предсказывал («должен быть добавлен как источник истины, а не только
 * жить в коде»): право проверялось здесь, но не было выдано НИ ОДНОЙ роли
 * в DEFAULT_ROLE_GRANTS — оба эндпоинта отвечали 403 всем и всегда, то
 * есть загрузить план этажа, фото юнита, документ агентства или аватар
 * было нельзя никак (property_photo закрыт отдельным путём property-assets
 * со своим правом). Найдено сверкой объявленных грантов с реальными
 * @RequirePermission-проверками. Грант выдан owner/director/rop/manager/
 * developer — тем, у кого уже есть property_asset.edit; обоснование — в
 * докстринге DEFAULT_ROLE_GRANTS.
 */
@Controller('media')
