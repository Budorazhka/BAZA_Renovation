...
export class ActualityService {
  ...
  /**
   * Батч-команда (owner decision xlsx #57) — САМА не является ни cron, ни
   * scheduler'ом (в кодовой базе по-прежнему нет ни одной scheduling-
   * зависимости, добавлять новую без решения владельца запрещено ТЗ).
   * Invokable-точка входа — `apps/api/src/jobs/actuality-expire.command.ts`
   * (`pnpm --filter api run actuality:expire-overdue`), тот же паттерн, что
   * apps/worker media-cleanup.command.ts: внешний OS-cron (ops, вне
   * кодовой базы, инфраструктурное решение при деплое, ещё не принято)
   * вызывает эту команду по расписанию. До появления такого cron-триггера
   * при деплое эта команда без ручного/внешнего вызова не выполняется сама
   * по себе — честный infrastructure gap, не симулированный. Для каждой
   * категории отдельно
