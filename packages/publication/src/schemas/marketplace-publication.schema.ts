...
export class MarketplacePublicationDocument extends Document {
  ...
  /**
   * Издатель заморожен админом (решение владельца 11.09.2026: заморозка
   * организации убирает её объявления с витрины и закрывает вход
   * сотрудникам, данные при этом сохраняются).
   *
   * Отдельный флаг, а не `status: 'unpublished'`, именно ради обратимости:
   * статус публикации — решение самого издателя, и затирать его
   * административной мерой значит потерять, что было опубликовано до
   * заморозки. Разморозка снимает флаг, и витрина возвращается в то же
   * состояние без участия издателя.
   */
  @Prop({ default: false })
  publisherFrozen!: boolean;
