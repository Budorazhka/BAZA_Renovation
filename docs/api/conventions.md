...
Освобождённые маршруты, где повтор всё же создаёт вторую сущность (создание ЖК,
корпуса, юнита, объекта, листинга, задачи, лида, сделки, media-asset, организации,
позиции, админ-аккаунта), помечены префиксом `ПРОБЕЛ:` — это принятый риск, а не
доказанная безопасность. Сейчас таких 19, число закреплено отдельной проверкой и
молча вырасти не может.
