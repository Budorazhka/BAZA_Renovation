...
> **Колонка «Статус готовности» в таблице ниже устарела.** Она заполнялась до
> решений владельца от 04.09.2026 и до того, как node ID были извлечены из `.fig`.
> Живой статус экранов — в реестре `figma-ui-delivery-gate.md`; здесь колонка
> оставлена как след того, что было неизвестно на момент составления.
>
> **Что изменилось 04.09.2026:**
>
> - `MKT-SCR-007` (десктоп ЖК) больше не `blocked`: владелец подтвердил, что
>   макет `3314:200742` живой, несмотря на пометку `(не используется)` в имени
>   узла.
> - `MKT-SCR-014` (рейтинг риэлторов): выбрана версия v2 `3576:53108`.
> - `MKT-SCR-003` (подвал): утверждён единый `footer 1` `3067:73634`.
> - `MKT-SCR-027` (служебные страницы): разблокирован и реализован — сборка по
>   токенам UI kit разрешена владельцем.
> - `MKT-SCR-025`, `MKT-SCR-026` (публичные профили застройщика и агентства):
>   **сняты из объёма**. Отдельных страниц не будет, клик по названию компании
>   ведёт в отфильтрованный каталог, как на действующем `baza.sale`. Реализовано,
>   см. `operations/marketplace-publisher-filter.md`.
> - `MKT-SCR-022`, `MKT-SCR-023`, `MKT-SCR-024` (встроенный CRM риэлтора):
>   **сняты из объёма** решением владельца, экраны удалены из кода.
> - Массовое `question` вида «извлечь числовые ID» закрыто: ID извлечены из `.fig`
>   и лежат в `figma-node-registry.json`.
>
> **Что осталось действительно открытым:** `MKT-SCR-013` (авторизация и
> регистрация — в Figma фрейма нет, решение о сборке из компонентов UI kit не
> принималось отдельно) и `MKT-SCR-021` (редактирование объявления — не начато).
