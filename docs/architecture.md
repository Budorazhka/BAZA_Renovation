...
- `apps/erp-web` — вендоренный клиент под версионированием этого репозитория.
  Пока явно исключён из pnpm-workspace и общего runtime Compose. Собственные
  тесты/сборка не заменяют включение в обязательные гейты. Задача подключения —
  P0-04 в [текущем плане](codex/plans/2026-09-07-primary-sales-completion.md).
