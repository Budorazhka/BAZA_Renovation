...
| `idempotency-coverage` | Список команд, требующих `Idempotency-Key` (conventions.md §8). |
...
1. **Страж, не проверенный на настоящей поломке, стражем не является.** Написав
   ...
   падает; затем восстановите код. Первая редакция `tenant-scope` искала
   `organizationId` в том числе в сигнатуре метода и потому пропускала самую
   вероятную форму бага: параметр принимается, но в фильтр не попадает.
2. **Список исключений — часть контракта.** У каждого исключения записана
