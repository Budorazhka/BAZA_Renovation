...
- [billing-manual-subscriptions.md](operations/billing-manual-subscriptions.md) — тарифы, подписка, журнал, ручная активация админом с грантом; с 11.09 обзор биллинга в админке тоже требует гранта (`manual_ledger.read`), не только вход администратора; лимиты и заморозка пока ничего не ограничивают.
