...
~~Нет кнопки "Выйти"~~ — устарело: кнопка "Выйти" есть в шапке
(`App.tsx::Shell::handleLogout`), вызывает реальный `POST /auth/logout`
(см. п.4 "Что не реализовано" ниже — закрыто `504ceb0`, исправлено
здесь 11.09.2026).
...
2. ~~**Деактивация admin-аккаунта из UI**~~ — закрыто веткой
   `codex/admin-lifecycle-hardening` (`504ceb0`, 29.08.2026): `POST
   /admin/accounts/:id/deactivate`/`.../reactivate`, см. "ОБНОВЛЕНО" в
   начале документа и таблицу маршрутов выше. Этот пункт списка не был
   обновлён тем же проходом — исправлено здесь 11.09.2026.
3. ~~**Отзыв (revoke) grant'а**~~ — закрыто тем же проходом (`504ceb0`):
   `POST /admin/accounts/:id/grants/:grantId/revoke`, см. таблицу
   маршрутов выше. Тот же случай неполного обновления списка.
4. ~~**Logout**~~ — закрыто тем же проходом: `POST /auth/logout`
   (`AuthController.logout`, `apps/api/src/modules/identity/auth.controller.ts`)
   существует, идемпотентен, чистит cookie и отзывает сессию — см.
   "ОБНОВЛЕНО" п.9 выше. Кнопка "Выйти" в admin-web существует
   (`App.tsx::handleLogout`). Тот же случай неполного обновления списка.
...
6. ~~**401 vs 403 на клиенте**~~ — закрыто тем же проходом (`504ceb0`):
   `AdminGuard` теперь различает `401 AUTH_NO_SESSION` (cookie вообще
   отсутствует) и `403 FORBIDDEN` (cookie есть, но не резолвится в
   валидный `AdminContext`), см. "401 vs 403" выше. admin-web
   по-прежнему трактует любой `403` от `GET /admin/me` как "не вошёл" —
   это осталось рабочей клиентской компенсацией, не багом. Тот же
   случай неполного обновления списка.
