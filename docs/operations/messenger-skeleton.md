...
2. `botToken` хранится открытым текстом без `select: false`
   (`messenger-account.schema.ts:40`): попадает в любой `find` и дамп базы. В ответы
   API сейчас не попадает.
3. `link-crm` не проверяет, что лид, контакт или сделка принадлежат организации и
   зоне видимости вызывающего, и не принимает `expectedVersion`.
4. Вложения: `assetId` не сверяется с организацией, `url` — любая строка.
5. Курсор списка диалогов фильтрует по `_id`, а сортировка идёт по закреплению и
   времени последнего сообщения: страницы теряют и дублируют диалоги.
6. Расхождения с OpenAPI: списки диалогов и сообщений отдают массив вместо
   `{items, nextCursor}`; `link-crm` отвечает 201 вместо 200.
7. Отметка прочтения меняет данные, но защищена правом `read`. Удаление аккаунта
   не удаляет его диалоги.
8. `create-task` использует ту же операцию идемпотентности `createTask`, что и
   `POST /tasks`: один ключ на двух эндпоинтах даст 409.
9. Организации, созданные до 10.09, не имеют грантов messenger, пока в окружении
